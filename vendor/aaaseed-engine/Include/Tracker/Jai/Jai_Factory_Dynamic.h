...
{
//maa	OutputDebugString(_T("OpenJaiSdk Called\r\n"));
		OutputDebugStringA( "OpenJaiSdk Called\r\n" );

#ifdef UNICODE
		
	TCHAR* path = _wgetenv( L"JAI_SDK_BIN_64" );
//	TCHAR* path = _tgetenv(_T("JAI_SDK_BIN_64"));
#else
//maa	TCHAR* path = _tgetenv(_T("JAI_SDK_BIN"));
	CHAR* path = getenv( "JAI_SDK_BIN" );
#endif
	if (path == NULL)
...
//maa	_stprintf(szPath,_T("%s\\Jai_Factory.dll"), path);
#ifdef UNICODE
	sprintf( szPath, "%ls\\Jai_Factory.dll", path);
#else
	sprintf( szPath, "%s\\Jai_Factory.dll", path);
#endif
	PUB_Handle = LoadLibraryA(szPath);
	if (PUB_Handle == NULL)
	{
		PUB_Handle = LoadLibraryA(szPath);
		return J_ST_FILE_IO;
	...
	{
//maa	OutputDebugString(_T("GetProcAddress() failed\r\n"));
...
//	OutputDebugString(_T("CloseJaiSdk Called\r\n"));
	OutputDebugStringA( "CloseJaiSdk Called\r\n" );
