...
//
//  VSS $Header: /PIDevTools/Inc/PDIdev.h 21    5/11/04 11:31a Sgagnon $  
//
/////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////
#ifndef _PDIDEV_H_
...
public:
	...
	BOOL	GetSDataList	( INT nSensor, CPDImdat &   );	  // Gets motion data output list per sensor ( 'O' Command )
	BOOL	GetSAlignment	( INT nSensor, PDI3vec & O, 
										   PDI3vec & X, 
										   PDI3vec & Y );	  // Gets Alignment Reference Frame per sensor ( 'A' Command )
	BOOL	GetSBoresight	( INT nSensor, PDIori & );		  // Get boresight per sensor ( 'B' Command )
	BOOL	GetSHemisphere	( INT nSensor, PDI3vec & );		  // Get Hemisphere Of Operation per sensor ( 'H' Command )
	BOOL	GetSHemiTrack	( INT nSensor, BOOL & );		  // Query Hemisphere Tracking per sensor ('H' Command )
	BOOL	GetSStylusMode	( INT nSensor, ePiStylusMode & ); // Get Stylus Mode per sensor ( 'L' Command )

	BOOL	ResetSAlignment ( INT nSensor );				  // Reset Alignment per sensor ( '^R' Command )

	BOOL	SetSDataList	( INT nSensor, const CPDImdat &   );		  // Sets motion data output list per sensor ( 'O' Command )
	BOOL	SetSAlignment	( INT nSensor, const PDI3vec & O, 
		                                   const PDI3vec & X, 
										   const PDI3vec & Y );			  // Sets Alignment Reference Frame per sensor ( 'A' Command )
	BOOL	SetSBoresight	( INT nSensor, const PDIori &, BOOL bReset ); // Set boresight per sensor ( 'B' Command )
	BOOL	SetSHemisphere	( INT nSensor, const PDI3vec & );			  // Set Hemisphere Of Operation per sensor ( 'H' Command )
	BOOL	SetSHemiTrack	( INT nSensor );							  // Enable Hemisphere Tracking per sensor ('H' Command )
	BOOL	SetSStylusMode	( INT nSensor, ePiStylusMode );				  // Set Stylus Mode per sensor ( 'L' Command )
	...
	PVOID	Trace		( BOOL );	// Enables trace output to debug window (DEBUG build only)
