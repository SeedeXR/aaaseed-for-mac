	...
	{																			\
		...
		{                                                                     	\
			...
			std::ostringstream dlib_o_out;										\
																				\
			ERR_PRINT_STRING( "Dlib detected at line ", __LINE__ );				\
			ERR_PRINT_STRING( "\t in file %.256s", __FILE__ );					\
 			ERR_PRINT_STRING( "\t in fn %.256s", DLIB_FUNCTION_NAME );			\
			ERR_PRINT_STRING( "\t Failing expression was  %.1024s", #_exp );	\
			dlib_o_out << std::boolalpha << _message;							\
			ERR_PRINT_STRING( "\t %.1024s", dlib_o_out.str().c_str() );			\
																				\
			dlib_o_out << "\n";													\
			throw dlib::error( dlib::EBROKEN_ASSERT, dlib_o_out.str() );		\
		}																		\
	}
#else
#define DLIBM_CASSERT(_exp,_message)                                              \
