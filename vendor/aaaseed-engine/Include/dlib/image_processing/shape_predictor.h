...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl
    {
		...
		if( b_dlib_maa )
		{
			unsigned long size = splits.size();
			while( i < size )
			{
				//maa
				auto& splits_i = splits[i];
				if ((float)feature_pixel_values[splits_i.idx1] - (float)feature_pixel_values[splits_i.idx2] > splits_i.thresh)
					i = left_child(i);
				else
					i = right_child(i);
			}
    ...
    // ------------------------------------------------------------------------------------
        ...
        {
	//maa
	    SPY_PUSH_RANGE( "dlib::extract_feature_pixel_values", spy::COL_1 );
		...
		if( b_dlib_maa )
		{		
			unsigned long size = feature_pixel_values.size();
    ...
    public:
        ...
        ) const
        {
            ...
            for (unsigned long iter = 0; iter < forests.size(); ++iter)
            {
				...
				//maa
				SPY_PUSH_RANGE( "dlib::evaluate trees", spy::COL_2 );
				if( b_dlib_maa )
				{
					auto& forests_iter = forests[iter];
					unsigned long size = forests_iter.size();
					for (unsigned long i = 0; i < size; ++i)
						current_shape += forests_iter[i](feature_pixel_values, leaf_idx);
				}
				else
				{
					for (unsigned long i = 0; i < forests[iter].size(); ++i)
            ...
            for (unsigned long iter = 0; iter < forests.size(); ++iter)
            {
				...
				if( b_dlib_maa )
				{
					...
					for (unsigned long i = 0; i < size; ++i)
					{
						unsigned long leaf_idx;
						//maa
						auto& forests_iter_i = forests_iter[i];
						current_shape += forests_iter_i(feature_pixel_values, leaf_idx);
						
						feats.push_back(std::make_pair(feat_offset+leaf_idx, 1));
						feat_offset += forests_iter_i.num_leaves();
					}
				}
				else
				{
