...
std::u16string toUtf16( const char *utf8Str, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
...
std::u32string toUtf32( const char *utf8Str, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
		lengthInBytes = strlen( utf8Str );
...
std::string toUtf8( const char16_t *utf16Str, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
...
std::string toUtf8( const char32_t *utf32Str, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
...
std::size_t stringLengthUtf8( const char *str, std::size_t lengthInBytes )
{
	std::size_t result{};
	std::size_t nextByte{};
	if( lengthInBytes == 0 )
...
{
	return stringLengthUtf8( str.data(), str.length() );
}

uint32_t nextCharUtf8( const char *str, std::size_t *inOutByte, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
		lengthInBytes = strlen( str );
...
std::size_t advanceCharUtf8( const char *str, std::size_t numChars, std::size_t lengthInBytes )
{
	if( lengthInBytes == 0 )
		lengthInBytes = strlen( str );
	std::size_t nextByte = 0;
	for( std::size_t curChar = 0; curChar < numChars; ++curChar )
	{
...
void lineBreakUtf8( const char *line, const std::function<bool( const char *, std::size_t )> &measureFn, const std::function<void( const char *, std::size_t )> &lineProcessFn )
{
	const std::size_t lengthInBytes = strlen( line );
	std::shared_ptr<char> brks = std::shared_ptr<char>( (char*)malloc( lengthInBytes ), free );
	...
	std::size_t charLen = stringLengthUtf8( line, lengthInBytes );
	std::size_t lineStartByte{};
	std::size_t lineEndByte{};
	std::size_t lineStartChar{};
	while( lineStartChar < charLen )
	{
		...
		if( brks.get()[ lineStartByte ] != 0 )
		{
			...
			std::size_t maxCharByte = lineStartByte;
			...
			while( minChar < maxChar )
			{
				...
				std::size_t newByte = advanceCharUtf8( line + lineStartByte, curChar, lengthInBytes );
		...
		{ // we started at a mustbreak
			curChar = 1;
		}

		// find ideal place to perform the break, either at curChar or before depending on breaks
		std::size_t lineEndByteAfterBreaking = lineEndByte = advanceCharUtf8( line + lineStartByte, curChar ) + lineStartByte;
		...
		std::size_t tempByte = lineStartByte;
