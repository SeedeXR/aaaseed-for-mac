...
/* to suit my needs ( cosmetic & memset & memcpy ) */
				...
				unsigned int inputLen)			/* length of input block */
{
	...
	if (inputLen >= partLen) {
		MEMCPY(&context->buffer[index], input, partLen, __FUNCTION__ );
	...
	MEMCPY(&context->buffer[index], &input[i],inputLen-i, __FUNCTION__ );
}
				...
				MD5_CTX *context)									/* context */
{
	...
	MEMCLEAR (context, sizeof (*context));
}
