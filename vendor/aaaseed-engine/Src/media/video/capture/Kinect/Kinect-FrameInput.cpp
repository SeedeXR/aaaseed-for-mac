...
namespace Kinect
{
	KinectFrameInput::KinectFrameInput(KinectFrameInputCallbacks *callbacks, usb_dev_handle* dev, unsigned char endpoint, int length_per_packet, int max_packets_in_buffer, int transfers_in_queue, int outputbuffersize)
	{
		...
		{
			int	ret = dll_libusb.usb_isochronous_setup_async(mDeviceHandle, &mTransfers[i], mEndPoint, mMaxActualPacketLength);
			if (ret<0)
			{
				ERR_PRINT_STRING( "error setting up isochronous request!" );	
			};
		...
		for (int i = 0;i<mMaxTransfers; ++i )
		{
			int ret = dll_libusb.usb_submit_async(mTransfers[i], (char*)&mPacketBuffers[i][0], mTransferSize);
			if (ret<0)
			{
				ERR_PRINT_STRING( "error submitting isochronous request!" );	
			};
		};
	};
	...
	KinectFrameInput::~KinectFrameInput()
	{
		for (int i = 0;i<mMaxTransfers; ++i )
		{
			dll_libusb.usb_cancel_async(mTransfers[i]);
			dll_libusb.usb_free_async(&mTransfers[i]);
			mTransfers[i] = nullptr;
		}
	...
	void KinectFrameInput::ProcessPacket(KinectUSBFrameHeader *header, unsigned char *data, int datalen)
	{
		...
		case 0x01:
			{
				...
				MEMCPY(mOutputBuffer, data, datalen, __FUNCTION__ );
		...
		case 0x02:
			{		
				if( b_valid )
				{
					...
					if (BytesToCopy > 0)
					{
						MEMCPY(mOutputBuffer+mWriteHeadPosition, data, BytesToCopy, __FUNCTION__ );
		...
		case 0x05:
			{				
				...
				{
					if( mCallbacks )
						mCallbacks->BufferComplete(this);
					mDropThisFrame = true;
				}
				else
				{
//					printf("frame dropped.. missing %d bytes!\n", mOutputBufferSize-mWriteHeadPosition);
	...
	{
		int UsbStatus = dll_libusb.usb_reap_async_nocancel(mTransfers[mCurrentTransfer], 10000);
		int RetVal = 0;
		if (UsbStatus>0)
		{
			RetVal = 1;
			...
			while (LeftOverBytes >0)
			{
				...
				if (CheckMagic(Header))
				{
					if (CurrentPacketLength<mMaxPacketLength)
					{
						...
						{
							ERR_PRINT_STRING( "incomplete packet of length %d found..", CurrentPacketLength);
						};
					}
					else
					{
		...
		else
		{
			if (UsbStatus<0)
			{
				...
				dll_libusb.usb_cancel_async(mTransfers[mCurrentTransfer]);
			}
		};
		ZeroMemory(&mPacketBuffers[mCurrentTransfer][0], mTransferSize);
		
		int ret = dll_libusb.usb_submit_async(mTransfers[mCurrentTransfer], (char*)&mPacketBuffers[mCurrentTransfer][0],  mTransferSize);
		if( ret < 0 )
		{
			ERR_PRINT_STRING( "error submitting async usb request: %s", dll_libusb.usb_strerror() );
			dll_libusb.usb_cancel_async(mTransfers[mCurrentTransfer]);
		}
	...
	bool KinectFrameInput::CheckSequence(KinectUSBFrameHeader *header)
	{
		...
		{
			//ERR_PRINT_STRING( "sequence lost: expected %02x, got %02x.", NewSequence, header->mSequence );
			return false;
		}
		return true;
	};
