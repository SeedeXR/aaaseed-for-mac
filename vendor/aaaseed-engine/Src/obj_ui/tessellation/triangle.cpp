...
#define deadtri(tria)  ((tria)[1] == (triangle) nullptr)

#define killtri(tria)	\
	(tria)[1] = (triangle) nullptr;	\
	(tria)[3] = (triangle) nullptr
...
#define deadsubseg(sub)  ((sub)[1] == (subseg) nullptr)

#define killsubseg(sub)	\
	(sub)[1] = (subseg) nullptr;	\
	(sub)[2] = (subseg) nullptr
...
#endif /* not ANSI_DECLARATORS */
{
	VOID *memptr= (VOID *) MALLOC( (unsigned int) size );
	if( memptr == nullptr )
	{
		ERR_PRINT_STRING("Error: Out of memory.");
		triexit(1);
	}
	return(memptr);
}
...
#ifdef CDT_ONLY
#ifdef REDUCED
	ERR_PRINT_STRING("triangle [-pAcjevngBPNEIOXzo_lQVh] input_file");
#else /* not REDUCED */
	ERR_PRINT_STRING("triangle [-pAcjevngBPNEIOXzo_iFlCQVh] input_file");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
	ERR_PRINT_STRING("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__lQVh] input_file");
#else /* not REDUCED */
	ERR_PRINT_STRING("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

	ERR_PRINT_STRING("    -p  Triangulates a Planar Straight Line Graph (.poly file).");
#ifndef CDT_ONLY
	ERR_PRINT_STRING("    -r  Refines a previously generated mesh.");
	ERR_PRINT_STRING("    -q  Quality mesh generation.  A minimum angle may be specified.");
	ERR_PRINT_STRING("    -a  Applies a maximum triangle area constraint.");
	ERR_PRINT_STRING("    -u  Applies a user-defined triangle constraint.");
#endif /* not CDT_ONLY */
	ERR_PRINT_STRING("    -A  Applies attributes to identify triangles in certain regions.");
	ERR_PRINT_STRING("    -c  Encloses the convex hull with segments.");
#ifndef CDT_ONLY
	ERR_PRINT_STRING("    -D  Conforming Delaunay:  all triangles are truly Delaunay.");
#endif /* not CDT_ONLY */
/*
	ERR_PRINT_STRING("    -w  Weighted Delaunay triangulation.");
	ERR_PRINT_STRING("    -W  Regular triangulation (lower hull of a height field).");
*/
	ERR_PRINT_STRING("    -j  Jettison unused vertices from output .node file.");
	ERR_PRINT_STRING("    -e  Generates an edge list.");
	ERR_PRINT_STRING("    -v  Generates a Voronoi diagram.");
	ERR_PRINT_STRING("    -n  Generates a list of triangle neighbors.");
	ERR_PRINT_STRING("    -g  Generates an .off file for Geomview.");
	ERR_PRINT_STRING("    -B  Suppresses output of boundary information.");
	ERR_PRINT_STRING("    -P  Suppresses output of .poly file.");
	ERR_PRINT_STRING("    -N  Suppresses output of .node file.");
	ERR_PRINT_STRING("    -E  Suppresses output of .ele file.");
	ERR_PRINT_STRING("    -I  Suppresses mesh iteration numbers.");
	ERR_PRINT_STRING("    -O  Ignores holes in .poly file.");
	ERR_PRINT_STRING("    -X  Suppresses use of exact arithmetic.");
	ERR_PRINT_STRING("    -z  Numbers all items starting from zero (rather than one).");
	ERR_PRINT_STRING("    -o2 Generates second-order subparametric elements.");
#ifndef CDT_ONLY
	ERR_PRINT_STRING("    -Y  Suppresses boundary segment splitting.");
	ERR_PRINT_STRING("    -S  Specifies maximum number of added Steiner points.");
#endif /* not CDT_ONLY */
#ifndef REDUCED
	ERR_PRINT_STRING("    -i  Uses incremental method, rather than divide-and-conquer.");
	ERR_PRINT_STRING("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.");
#endif /* not REDUCED */
	ERR_PRINT_STRING("    -l  Uses vertical cuts only, rather than alternating cuts.");
#ifndef REDUCED
#ifndef CDT_ONLY
	ERR_PRINT_STRING("    -s  Force segments into mesh by splitting (instead of using CDT).");
#endif /* not CDT_ONLY */
	ERR_PRINT_STRING("    -C  Check consistency of final mesh.");
#endif /* not REDUCED */
	ERR_PRINT_STRING("    -Q  Quiet:  No terminal output except errors.");
	ERR_PRINT_STRING("    -V  Verbose:  Detailed information on what I'm doing.");
	ERR_PRINT_STRING("    -h  Help:  Detailed instructions for Triangle.");
	triexit(0);
}
...
void info()
{
	ERR_PRINT_STRING("Triangle");
	ERR_PRINT_STRING("A Two-Dimensional Quality Mesh Generator and Delaunay Triangulator.");
	ERR_PRINT_STRING("Version 1.6\n");
	ERR_PRINT_STRING("Copyright 1993, 1995, 1997, 1998, 2002, 2005 Jonathan Richard Shewchuk");
	ERR_PRINT_STRING("2360 Woolsey #H / Berkeley, California 94705-1927");
	ERR_PRINT_STRING("Bugs/comments to jrs@cs.berkeley.edu");
	ERR_PRINT_STRING("Created as part of the Quake project (tools for earthquake simulation).");
	ERR_PRINT_STRING("Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 Scholarship.");
	ERR_PRINT_STRING("There is no warranty whatsoever.  Use at your own risk.");
#ifdef SINGLE
	ERR_PRINT_STRING("This executable is compiled for single precision arithmetic.\n\n");
#else /* not SINGLE */
	ERR_PRINT_STRING("This executable is compiled for double precision arithmetic.\n\n");
#endif /* not SINGLE */
	ERR_PRINT_STRING("Triangle generates exact Delaunay triangulations, constrained Delaunay");
	ERR_PRINT_STRING("triangulations, conforming Delaunay triangulations, Voronoi diagrams, and");
	ERR_PRINT_STRING("high-quality triangular meshes.  The latter can be generated with no small");
	ERR_PRINT_STRING("or large angles, and are thus suitable for finite element analysis.  If no");
	ERR_PRINT_STRING("command line switch is specified, your .node input file is read, and the");
	ERR_PRINT_STRING("Delaunay triangulation is returned in .node and .ele output files.  The");
	ERR_PRINT_STRING("command syntax is:\n");
	ERR_PRINT_STRING("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
	ERR_PRINT_STRING("Underscores indicate that numbers may optionally follow certain switches.");
	ERR_PRINT_STRING("Do not leave any space between a switch and its numeric parameter.");
	ERR_PRINT_STRING("input_file must be a file with extension .node, or extension .poly if the");
	ERR_PRINT_STRING("-p switch is used.  If -r is used, you must supply .node and .ele files,");
	ERR_PRINT_STRING("and possibly a .poly file and an .area file as well.  The formats of these");
	ERR_PRINT_STRING("files are described below.\n");
	ERR_PRINT_STRING("Command Line Switches:\n");
	ERR_PRINT_STRING("    -p  Reads a Planar Straight Line Graph (.poly file), which can specify");
	ERR_PRINT_STRING("        vertices, segments, holes, regional attributes, and regional area");
	ERR_PRINT_STRING("        constraints.  Generates a constrained Delaunay triangulation (CDT)");
	ERR_PRINT_STRING("        fitting the input; or, if -s, -q, -a, or -u is used, a conforming");
	ERR_PRINT_STRING("        constrained Delaunay triangulation (CCDT).  If you want a truly");
	ERR_PRINT_STRING("        Delaunay (not just constrained Delaunay) triangulation, use -D as");
	ERR_PRINT_STRING("        well.  When -p is not used, Triangle reads a .node file by default.");
	ERR_PRINT_STRING("    -r  Refines a previously generated mesh.  The mesh is read from a .node");
	ERR_PRINT_STRING("        file and an .ele file.  If -p is also used, a .poly file is read");
	ERR_PRINT_STRING("        and used to constrain segments in the mesh.  If -a is also used");
	ERR_PRINT_STRING("        (with no number following), an .area file is read and used to");
	ERR_PRINT_STRING("        impose area constraints on the mesh.  Further details on refinement");
	ERR_PRINT_STRING("        appear below.");
	ERR_PRINT_STRING("    -q  Quality mesh generation by Delaunay refinement (a hybrid of Paul");
	ERR_PRINT_STRING("        Chew's and Jim Ruppert's algorithms).  Adds vertices to the mesh to");
	ERR_PRINT_STRING("        ensure that all angles are between 20 and 140 degrees.  An");
	ERR_PRINT_STRING("        alternative bound on the minimum angle, replacing 20 degrees, may");
	ERR_PRINT_STRING("        be specified after the `q'.  The specified angle may include a");
	ERR_PRINT_STRING("        decimal point, but not exponential notation.  Note that a bound of");
	ERR_PRINT_STRING("        theta degrees on the smallest angle also implies a bound of");
	ERR_PRINT_STRING("        (180 - 2 theta) on the largest angle.  If the minimum angle is 28.6");
	ERR_PRINT_STRING("        degrees or smaller, Triangle is mathematically guaranteed to");
	ERR_PRINT_STRING("        terminate (assuming infinite precision arithmetic--Triangle may");
	ERR_PRINT_STRING("        fail to terminate if you run out of precision).  In practice,");
	ERR_PRINT_STRING("        Triangle often succeeds for minimum angles up to 34 degrees.  For");
	ERR_PRINT_STRING("        some meshes, however, you might need to reduce the minimum angle to");
	ERR_PRINT_STRING("        avoid problems associated with insufficient floating-point");
	ERR_PRINT_STRING("        precision.");
	ERR_PRINT_STRING("    -a  Imposes a maximum triangle area.  If a number follows the `a', no");
	ERR_PRINT_STRING("        triangle is generated whose area is larger than that number.  If no");
	ERR_PRINT_STRING("        number is specified, an .area file (if -r is used) or .poly file");
	ERR_PRINT_STRING("        (if -r is not used) specifies a set of maximum area constraints.");
	ERR_PRINT_STRING("        An .area file contains a separate area constraint for each");
	ERR_PRINT_STRING("        triangle, and is useful for refining a finite element mesh based on");
	ERR_PRINT_STRING("        a posteriori error estimates.  A .poly file can optionally contain");
	ERR_PRINT_STRING("        an area constraint for each segment-bounded region, thereby");
	ERR_PRINT_STRING("        controlling triangle densities in a first triangulation of a PSLG.");
	ERR_PRINT_STRING("        You can impose both a fixed area constraint and a varying area");
	ERR_PRINT_STRING("        constraint by invoking the -a switch twice, once with and once");
	ERR_PRINT_STRING("        without a number following.  Each area specified may include a");
	ERR_PRINT_STRING("        decimal point.");
	ERR_PRINT_STRING("    -u  Imposes a user-defined constraint on triangle size.  There are two");
	ERR_PRINT_STRING("        ways to use this feature.  One is to edit the triunsuitable()");
	ERR_PRINT_STRING("        procedure in triangle.c to encode any constraint you like, then");
	ERR_PRINT_STRING("        recompile Triangle.  The other is to compile triangle.c with the");
	ERR_PRINT_STRING("        EXTERNAL_TEST symbol set (compiler switch -DEXTERNAL_TEST), then");
	ERR_PRINT_STRING("        link Triangle with a separate object file that implements");
	ERR_PRINT_STRING("        triunsuitable().  In either case, the -u switch causes the user-");
	ERR_PRINT_STRING("        defined test to be applied to every triangle.");
	ERR_PRINT_STRING("    -A  Assigns an additional floating-point attribute to each triangle");
	ERR_PRINT_STRING("        that identifies what segment-bounded region each triangle belongs");
	ERR_PRINT_STRING("        to.  Attributes are assigned to regions by the .poly file.  If a");
	ERR_PRINT_STRING("        region is not explicitly marked by the .poly file, triangles in");
	ERR_PRINT_STRING("        that region are assigned an attribute of zero.  The -A switch has");
	ERR_PRINT_STRING("        an effect only when the -p switch is used and the -r switch is not.");
	ERR_PRINT_STRING("    -c  Creates segments on the convex hull of the triangulation.  If you");
	ERR_PRINT_STRING("        are triangulating a vertex set, this switch causes a .poly file to");
	ERR_PRINT_STRING("        be written, containing all edges of the convex hull.  If you are");
	ERR_PRINT_STRING("        triangulating a PSLG, this switch specifies that the whole convex");
	ERR_PRINT_STRING("        hull of the PSLG should be triangulated, regardless of what");
	ERR_PRINT_STRING("        segments the PSLG has.  If you do not use this switch when");
	ERR_PRINT_STRING("        triangulating a PSLG, Triangle assumes that you have identified the");
	ERR_PRINT_STRING("        region to be triangulated by surrounding it with segments of the");
	ERR_PRINT_STRING("        input PSLG.  Beware:  if you are not careful, this switch can cause");
	ERR_PRINT_STRING("        the introduction of an extremely thin angle between a PSLG segment");
	ERR_PRINT_STRING("        and a convex hull segment, which can cause overrefinement (and");
	ERR_PRINT_STRING("        possibly failure if Triangle runs out of precision).  If you are");
	ERR_PRINT_STRING("        refining a mesh, the -c switch works differently:  it causes a");
	ERR_PRINT_STRING("        .poly file to be written containing the boundary edges of the mesh");
	ERR_PRINT_STRING("        (useful if no .poly file was read).");
	ERR_PRINT_STRING("    -D  Conforming Delaunay triangulation:  use this switch if you want to");
	ERR_PRINT_STRING("        ensure that all the triangles in the mesh are Delaunay, and not");
	ERR_PRINT_STRING("        merely constrained Delaunay; or if you want to ensure that all the");
	ERR_PRINT_STRING("        Voronoi vertices lie within the triangulation.  (Some finite volume");
	ERR_PRINT_STRING("        methods have this requirement.)  This switch invokes Ruppert's");
	ERR_PRINT_STRING("        original algorithm, which splits every subsegment whose diametral");
	ERR_PRINT_STRING("        circle is encroached.  It usually increases the number of vertices");
	ERR_PRINT_STRING("        and triangles.");
	ERR_PRINT_STRING("    -j  Jettisons vertices that are not part of the final triangulation");
	ERR_PRINT_STRING("        from the output .node file.  By default, Triangle copies all");
	ERR_PRINT_STRING("        vertices in the input .node file to the output .node file, in the");
	ERR_PRINT_STRING("        same order, so their indices do not change.  The -j switch prevents");
	ERR_PRINT_STRING("        duplicated input vertices, or vertices `eaten' by holes, from");
	ERR_PRINT_STRING("        appearing in the output .node file.  Thus, if two input vertices");
	ERR_PRINT_STRING("        have exactly the same coordinates, only the first appears in the");
	ERR_PRINT_STRING("        output.  If any vertices are jettisoned, the vertex numbering in");
	ERR_PRINT_STRING("        the output .node file differs from that of the input .node file.");
	ERR_PRINT_STRING("    -e  Outputs (to an .edge file) a list of edges of the triangulation.");
	ERR_PRINT_STRING("    -v  Outputs the Voronoi diagram associated with the triangulation.");
	ERR_PRINT_STRING("        Does not attempt to detect degeneracies, so some Voronoi vertices");
	ERR_PRINT_STRING("        may be duplicated.  See the discussion of Voronoi diagrams below.");
	ERR_PRINT_STRING("    -n  Outputs (to a .neigh file) a list of triangles neighboring each");
	ERR_PRINT_STRING("        triangle.");
	ERR_PRINT_STRING("    -g  Outputs the mesh to an Object File Format (.off) file, suitable for");
	ERR_PRINT_STRING("        viewing with the Geometry Center's Geomview package.");
	ERR_PRINT_STRING("    -B  No boundary markers in the output .node, .poly, and .edge output");
	ERR_PRINT_STRING("        files.  See the detailed discussion of boundary markers below.");
	ERR_PRINT_STRING("    -P  No output .poly file.  Saves disk space, but you lose the ability");
	ERR_PRINT_STRING("        to maintain constraining segments on later refinements of the mesh.");
	ERR_PRINT_STRING("    -N  No output .node file.");
	ERR_PRINT_STRING("    -E  No output .ele file.");
	ERR_PRINT_STRING("    -I  No iteration numbers.  Suppresses the output of .node and .poly");
	ERR_PRINT_STRING("        files, so your input files won't be overwritten.  (If your input is");
	ERR_PRINT_STRING("        a .poly file only, a .node file is written.)  Cannot be used with");
	ERR_PRINT_STRING("        the -r switch, because that would overwrite your input .ele file.");
	ERR_PRINT_STRING("        Shouldn't be used with the -q, -a, -u, or -s switch if you are");
	ERR_PRINT_STRING("        using a .node file for input, because no .node file is written, so");
	ERR_PRINT_STRING("        there is no record of any added Steiner points.");
	ERR_PRINT_STRING("    -O  No holes.  Ignores the holes in the .poly file.");
	ERR_PRINT_STRING("    -X  No exact arithmetic.  Normally, Triangle uses exact floating-point");
	ERR_PRINT_STRING("        arithmetic for certain tests if it thinks the inexact tests are not");
	ERR_PRINT_STRING("        accurate enough.  Exact arithmetic ensures the robustness of the");
	ERR_PRINT_STRING("        triangulation algorithms, despite floating-point roundoff error.");
	ERR_PRINT_STRING("        Disabling exact arithmetic with the -X switch causes a small");
	ERR_PRINT_STRING("        improvement in speed and creates the possibility that Triangle will");
	ERR_PRINT_STRING("        fail to produce a valid mesh.  Not recommended.");
	ERR_PRINT_STRING("    -z  Numbers all items starting from zero (rather than one).  Note that");
	ERR_PRINT_STRING("        this switch is normally overridden by the value used to number the");
	ERR_PRINT_STRING("        first vertex of the input .node or .poly file.  However, this");
	ERR_PRINT_STRING("        switch is useful when calling Triangle from another program.");
	ERR_PRINT_STRING("    -o2 Generates second-order subparametric elements with six nodes each.");
	ERR_PRINT_STRING("    -Y  No new vertices on the boundary.  This switch is useful when the");
	ERR_PRINT_STRING("        mesh boundary must be preserved so that it conforms to some");
	ERR_PRINT_STRING("        adjacent mesh.  Be forewarned that you will probably sacrifice much");
	ERR_PRINT_STRING("        of the quality of the mesh; Triangle will try, but the resulting");
	ERR_PRINT_STRING("        mesh may contain poorly shaped triangles.  Works well if all the");
	ERR_PRINT_STRING("        boundary vertices are closely spaced.  Specify this switch twice");
	ERR_PRINT_STRING("        (`-YY') to prevent all segment splitting, including internal");
	ERR_PRINT_STRING("        boundaries.");
	ERR_PRINT_STRING("    -S  Specifies the maximum number of Steiner points (vertices that are");
	ERR_PRINT_STRING("        not in the input, but are added to meet the constraints on minimum");
	ERR_PRINT_STRING("        angle and maximum area).  The default is to allow an unlimited");
	ERR_PRINT_STRING("        number.  If you specify this switch with no number after it,");
	ERR_PRINT_STRING("        the limit is set to zero.  Triangle always adds vertices at segment");
	ERR_PRINT_STRING("        intersections, even if it needs to use more vertices than the limit");
	ERR_PRINT_STRING("        you set.  When Triangle inserts segments by splitting (-s), it");
	ERR_PRINT_STRING("        always adds enough vertices to ensure that all the segments of the");
	ERR_PRINT_STRING("        PLSG are recovered, ignoring the limit if necessary.");
	ERR_PRINT_STRING("    -i  Uses an incremental rather than a divide-and-conquer algorithm to");
	ERR_PRINT_STRING("        construct a Delaunay triangulation.  Try it if the divide-and-");
	ERR_PRINT_STRING("        conquer algorithm fails.");
	ERR_PRINT_STRING("    -F  Uses Steven Fortune's sweepline algorithm to construct a Delaunay");
	ERR_PRINT_STRING("        triangulation.  Warning:  does not use exact arithmetic for all");
	ERR_PRINT_STRING("        calculations.  An exact result is not guaranteed.");
	ERR_PRINT_STRING("    -l  Uses only vertical cuts in the divide-and-conquer algorithm.  By");
	ERR_PRINT_STRING("        default, Triangle alternates between vertical and horizontal cuts,");
	ERR_PRINT_STRING("        which usually improve the speed except with vertex sets that are");
	ERR_PRINT_STRING("        small or short and wide.  This switch is primarily of theoretical");
	ERR_PRINT_STRING("        interest.");
	ERR_PRINT_STRING("    -s  Specifies that segments should be forced into the triangulation by");
	ERR_PRINT_STRING("        recursively splitting them at their midpoints, rather than by");
	ERR_PRINT_STRING("        generating a constrained Delaunay triangulation.  Segment splitting");
	ERR_PRINT_STRING("        is true to Ruppert's original algorithm, but can create needlessly");
	ERR_PRINT_STRING("        small triangles.  This switch is primarily of theoretical interest.");
	ERR_PRINT_STRING("    -C  Check the consistency of the final mesh.  Uses exact arithmetic for");
	ERR_PRINT_STRING("        checking, even if the -X switch is used.  Useful if you suspect");
	ERR_PRINT_STRING("        Triangle is buggy.");
	ERR_PRINT_STRING("    -Q  Quiet:  Suppresses all explanation of what Triangle is doing,");
	ERR_PRINT_STRING("        unless an error occurs.");
	ERR_PRINT_STRING("    -V  Verbose:  Gives detailed information about what Triangle is doing.");
	ERR_PRINT_STRING("        Add more `V's for increasing amount of detail.  `-V' is most");
	ERR_PRINT_STRING("        useful; itgives information on algorithmic progress and much more");
	ERR_PRINT_STRING("        detailed statistics.  `-VV' gives vertex-by-vertex details, and");
	ERR_PRINT_STRING("        prints so much that Triangle runs much more slowly.  `-VVVV' gives");
	ERR_PRINT_STRING("        information only a debugger could love.");
	ERR_PRINT_STRING("    -h  Help:  Displays these instructions.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("Definitions:");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  A Delaunay triangulation of a vertex set is a triangulation whose");
	ERR_PRINT_STRING("  vertices are the vertex set, that covers the convex hull of the vertex");
	ERR_PRINT_STRING("  set.  A Delaunay triangulation has the property that no vertex lies");
	ERR_PRINT_STRING("  inside the circumscribing circle (circle that passes through all three");
	ERR_PRINT_STRING("  vertices) of any triangle in the triangulation.\n");
	ERR_PRINT_STRING("  A Voronoi diagram of a vertex set is a subdivision of the plane into");
	ERR_PRINT_STRING("  polygonal cells (some of which may be unbounded, meaning infinitely");
	ERR_PRINT_STRING("  large), where each cell is the set of points in the plane that are closer");
	ERR_PRINT_STRING("  to some input vertex than to any other input vertex.  The Voronoi diagram");
	ERR_PRINT_STRING("  is a geometric dual of the Delaunay triangulation.\n");
	ERR_PRINT_STRING("  A Planar Straight Line Graph (PSLG) is a set of vertices and segments.");
	ERR_PRINT_STRING("  Segments are simply edges, whose endpoints are all vertices in the PSLG.");
	ERR_PRINT_STRING("  Segments may intersect each other only at their endpoints.  The file");
	ERR_PRINT_STRING("  format for PSLGs (.poly files) is described below.\n");
	ERR_PRINT_STRING("  A constrained Delaunay triangulation (CDT) of a PSLG is similar to a");
	ERR_PRINT_STRING("  Delaunay triangulation, but each PSLG segment is present as a single edge");
	ERR_PRINT_STRING("  of the CDT.  (A constrained Delaunay triangulation is not truly a");
	ERR_PRINT_STRING("  Delaunay triangulation, because some of its triangles might not be");
	ERR_PRINT_STRING("  Delaunay.)  By definition, a CDT does not have any vertices other than");
	ERR_PRINT_STRING("  those specified in the input PSLG.  Depending on context, a CDT might");
	ERR_PRINT_STRING("  cover the convex hull of the PSLG, or it might cover only a segment-");
	ERR_PRINT_STRING("  bounded region (e.g. a polygon).\n");
	ERR_PRINT_STRING("  A conforming Delaunay triangulation of a PSLG is a triangulation in which");
	ERR_PRINT_STRING("  each triangle is truly Delaunay, and each PSLG segment is represented by");
	ERR_PRINT_STRING("  a linear contiguous sequence of edges of the triangulation.  New vertices");
	ERR_PRINT_STRING("  (not part of the PSLG) may appear, and each input segment may have been");
	ERR_PRINT_STRING("  subdivided into shorter edges (subsegments) by these additional vertices.");
	ERR_PRINT_STRING("  The new vertices are frequently necessary to maintain the Delaunay");
	ERR_PRINT_STRING("  property while ensuring that every segment is represented.\n");
	ERR_PRINT_STRING("  A conforming constrained Delaunay triangulation (CCDT) of a PSLG is a");
	ERR_PRINT_STRING("  triangulation of a PSLG whose triangles are constrained Delaunay.  New");
	ERR_PRINT_STRING("  vertices may appear, and input segments may be subdivided into");
	ERR_PRINT_STRING("  subsegments, but not to guarantee that segments are respected; rather, to");
	ERR_PRINT_STRING("  improve the quality of the triangles.  The high-quality meshes produced");
	ERR_PRINT_STRING("  by the -q switch are usually CCDTs, but can be made conforming Delaunay");
	ERR_PRINT_STRING("  with the -D switch.\n");
	ERR_PRINT_STRING("File Formats:\n");
	ERR_PRINT_STRING("  All files may contain comments prefixed by the character '#'.  Vertices,");
	ERR_PRINT_STRING("  triangles, edges, holes, and maximum area constraints must be numbered");
	ERR_PRINT_STRING("  consecutively, starting from either 1 or 0.  Whichever you choose, all");
	ERR_PRINT_STRING("  input files must be consistent; if the vertices are numbered from 1, so");
	ERR_PRINT_STRING("  must be all other objects.  Triangle automatically detects your choice");
	ERR_PRINT_STRING("  while reading the .node (or .poly) file.  (When calling Triangle from");
	ERR_PRINT_STRING("  another program, use the -z switch if you wish to number objects from");
	ERR_PRINT_STRING("  zero.)  Examples of these file formats are given below.\n");
	ERR_PRINT_STRING("  .node files:");
	ERR_PRINT_STRING("    First line:  <# of vertices> <dimension (must be 2)> <# of attributes>");
	ERR_PRINT_STRING("                                           <# of boundary markers (0 or 1)>");
	ERR_PRINT_STRING("    Remaining lines:  <vertex #> <x> <y> [attributes] [boundary marker]");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    The attributes, which are typically floating-point values of physical");
	ERR_PRINT_STRING("    quantities (such as mass or conductivity) associated with the nodes of");
	ERR_PRINT_STRING("    a finite element mesh, are copied unchanged to the output mesh.  If -q,");
	ERR_PRINT_STRING("    -a, -u, -D, or -s is selected, each new Steiner point added to the mesh");
	ERR_PRINT_STRING("    has attributes assigned to it by linear interpolation.\n");
	ERR_PRINT_STRING("    If the fourth entry of the first line is `1', the last column of the");
	ERR_PRINT_STRING("    remainder of the file is assumed to contain boundary markers.  Boundary");
	ERR_PRINT_STRING("    markers are used to identify boundary vertices and vertices resting on");
	ERR_PRINT_STRING("    PSLG segments; a complete description appears in a section below.  The");
	ERR_PRINT_STRING("    .node file produced by Triangle contains boundary markers in the last");
	ERR_PRINT_STRING("    column unless they are suppressed by the -B switch.\n");
	ERR_PRINT_STRING("  .ele files:");
	ERR_PRINT_STRING("    First line:  <# of triangles> <nodes per triangle> <# of attributes>");
	ERR_PRINT_STRING("    Remaining lines:  <triangle #> <node> <node> <node> ... [attributes]");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    Nodes are indices into the corresponding .node file.  The first three");
	ERR_PRINT_STRING("    nodes are the corner vertices, and are listed in counterclockwise order");
	ERR_PRINT_STRING("    around each triangle.  (The remaining nodes, if any, depend on the type");
	ERR_PRINT_STRING("    of finite element used.)\n");
	ERR_PRINT_STRING("    The attributes are just like those of .node files.  Because there is no");
	ERR_PRINT_STRING("    simple mapping from input to output triangles, Triangle attempts to");
	ERR_PRINT_STRING("    interpolate attributes, and may cause a lot of diffusion of attributes");
	ERR_PRINT_STRING("    among nearby triangles as the triangulation is refined.  Attributes do");
	ERR_PRINT_STRING("    not diffuse across segments, so attributes used to identify");
	ERR_PRINT_STRING("    segment-bounded regions remain intact.\n");
	ERR_PRINT_STRING("    In .ele files produced by Triangle, each triangular element has three");
	ERR_PRINT_STRING("    nodes (vertices) unless the -o2 switch is used, in which case");
	ERR_PRINT_STRING("    subparametric quadratic elements with six nodes each are generated.");
	ERR_PRINT_STRING("    The first three nodes are the corners in counterclockwise order, and");
	ERR_PRINT_STRING("    the fourth, fifth, and sixth nodes lie on the midpoints of the edges");
	ERR_PRINT_STRING("    opposite the first, second, and third vertices, respectively.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  .poly files:");
	ERR_PRINT_STRING("    First line:  <# of vertices> <dimension (must be 2)> <# of attributes>");
	ERR_PRINT_STRING("                                           <# of boundary markers (0 or 1)>");
	ERR_PRINT_STRING("    Following lines:  <vertex #> <x> <y> [attributes] [boundary marker]");
	ERR_PRINT_STRING("    One line:  <# of segments> <# of boundary markers (0 or 1)>");
	ERR_PRINT_STRING("    Following lines:  <segment #> <endpoint> <endpoint> [boundary marker]");
	ERR_PRINT_STRING("    One line:  <# of holes>");
	ERR_PRINT_STRING("    Following lines:  <hole #> <x> <y>");
	ERR_PRINT_STRING("    Optional line:  <# of regional attributes and/or area constraints>");
	ERR_PRINT_STRING("    Optional following lines:  <region #> <x> <y> <attribute> <max area>");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    A .poly file represents a PSLG, as well as some additional information.");
	ERR_PRINT_STRING("    The first section lists all the vertices, and is identical to the");
	ERR_PRINT_STRING("    format of .node files.  <# of vertices> may be set to zero to indicate");
	ERR_PRINT_STRING("    that the vertices are listed in a separate .node file; .poly files");
	ERR_PRINT_STRING("    produced by Triangle always have this format.  A vertex set represented");
	ERR_PRINT_STRING("    this way has the advantage that it may easily be triangulated with or");
	ERR_PRINT_STRING("    without segments (depending on whether the -p switch is invoked).");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    The second section lists the segments.  Segments are edges whose");
	ERR_PRINT_STRING("    presence in the triangulation is enforced.  (Depending on the choice of");
	ERR_PRINT_STRING("    switches, segment might be subdivided into smaller edges).  Each");
	ERR_PRINT_STRING("    segment is specified by listing the indices of its two endpoints.  This");
	ERR_PRINT_STRING("    means that you must include its endpoints in the vertex list.  Each");
	ERR_PRINT_STRING("    segment, like each point, may have a boundary marker.\n");
	ERR_PRINT_STRING("    If -q, -a, -u, and -s are not selected, Triangle produces a constrained");
	ERR_PRINT_STRING("    Delaunay triangulation (CDT), in which each segment appears as a single");
	ERR_PRINT_STRING("    edge in the triangulation.  If -q, -a, -u, or -s is selected, Triangle");
	ERR_PRINT_STRING("    produces a conforming constrained Delaunay triangulation (CCDT), in");
	ERR_PRINT_STRING("    which segments may be subdivided into smaller edges.  If -D is");
	ERR_PRINT_STRING("    selected, Triangle produces a conforming Delaunay triangulation, so");
	ERR_PRINT_STRING("    that every triangle is Delaunay, and not just constrained Delaunay.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    The third section lists holes (and concavities, if -c is selected) in");
	ERR_PRINT_STRING("    the triangulation.  Holes are specified by identifying a point inside");
	ERR_PRINT_STRING("    each hole.  After the triangulation is formed, Triangle creates holes");
	ERR_PRINT_STRING("    by eating triangles, spreading out from each hole point until its");
	ERR_PRINT_STRING("    progress is blocked by segments in the PSLG.  You must be careful to");
	ERR_PRINT_STRING("    enclose each hole in segments, or your whole triangulation might be");
	ERR_PRINT_STRING("    eaten away.  If the two triangles abutting a segment are eaten, the");
	ERR_PRINT_STRING("    segment itself is also eaten.  Do not place a hole directly on a");
	ERR_PRINT_STRING("    segment; if you do, Triangle chooses one side of the segment");
	ERR_PRINT_STRING("    arbitrarily.\n");
	ERR_PRINT_STRING("    The optional fourth section lists regional attributes (to be assigned");
	ERR_PRINT_STRING("    to all triangles in a region) and regional constraints on the maximum");
	ERR_PRINT_STRING("    triangle area.  Triangle reads this section only if the -A switch is");
	ERR_PRINT_STRING("    used or the -a switch is used without a number following it, and the -r");
	ERR_PRINT_STRING("    switch is not used.  Regional attributes and area constraints are");
	ERR_PRINT_STRING("    propagated in the same manner as holes:  you specify a point for each");
	ERR_PRINT_STRING("    attribute and/or constraint, and the attribute and/or constraint");
	ERR_PRINT_STRING("    affects the whole region (bounded by segments) containing the point.");
	ERR_PRINT_STRING("    If two values are written on a line after the x and y coordinate, the");
	ERR_PRINT_STRING("    first such value is assumed to be a regional attribute (but is only");
	ERR_PRINT_STRING("    applied if the -A switch is selected), and the second value is assumed");
	ERR_PRINT_STRING("    to be a regional area constraint (but is only applied if the -a switch");
	ERR_PRINT_STRING("    is selected).  You may specify just one value after the coordinates,");
	ERR_PRINT_STRING("    which can serve as both an attribute and an area constraint, depending");
	ERR_PRINT_STRING("    on the choice of switches.  If you are using the -A and -a switches");
	ERR_PRINT_STRING("    simultaneously and wish to assign an attribute to some region without");
	ERR_PRINT_STRING("    imposing an area constraint, use a negative maximum area.\n");
	ERR_PRINT_STRING("    When a triangulation is created from a .poly file, you must either");
	ERR_PRINT_STRING("    enclose the entire region to be triangulated in PSLG segments, or");
	ERR_PRINT_STRING("    use the -c switch, which automatically creates extra segments that");
	ERR_PRINT_STRING("    enclose the convex hull of the PSLG.  If you do not use the -c switch,");
	ERR_PRINT_STRING("    Triangle eats all triangles that are not enclosed by segments; if you");
	ERR_PRINT_STRING("    are not careful, your whole triangulation may be eaten away.  If you do");
	ERR_PRINT_STRING("    use the -c switch, you can still produce concavities by the appropriate");
	ERR_PRINT_STRING("    placement of holes just inside the boundary of the convex hull.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    An ideal PSLG has no intersecting segments, nor any vertices that lie");
	ERR_PRINT_STRING("    upon segments (except, of course, the endpoints of each segment).  You");
	ERR_PRINT_STRING("    aren't required to make your .poly files ideal, but you should be aware");
	ERR_PRINT_STRING("    of what can go wrong.  Segment intersections are relatively safe--");
	ERR_PRINT_STRING("    Triangle calculates the intersection points for you and adds them to");
	ERR_PRINT_STRING("    the triangulation--as long as your machine's floating-point precision");
	ERR_PRINT_STRING("    doesn't become a problem.  You are tempting the fates if you have three");
	ERR_PRINT_STRING("    segments that cross at the same location, and expect Triangle to figure");
	ERR_PRINT_STRING("    out where the intersection point is.  Thanks to floating-point roundoff");
	ERR_PRINT_STRING("    error, Triangle will probably decide that the three segments intersect");
	ERR_PRINT_STRING("    at three different points, and you will find a minuscule triangle in");
	ERR_PRINT_STRING("    your output--unless Triangle tries to refine the tiny triangle, uses");
	ERR_PRINT_STRING("    up the last bit of machine precision, and fails to terminate at all.");
	ERR_PRINT_STRING("    You're better off putting the intersection point in the input files,");
	ERR_PRINT_STRING("    and manually breaking up each segment into two.  Similarly, if you");
	ERR_PRINT_STRING("    place a vertex at the middle of a segment, and hope that Triangle will");
	ERR_PRINT_STRING("    break up the segment at that vertex, you might get lucky.  On the other");
	ERR_PRINT_STRING("    hand, Triangle might decide that the vertex doesn't lie precisely on");
	ERR_PRINT_STRING("    the segment, and you'll have a needle-sharp triangle in your output--or");
	ERR_PRINT_STRING("    a lot of tiny triangles if you're generating a quality mesh.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    When Triangle reads a .poly file, it also writes a .poly file, which");
	ERR_PRINT_STRING("    includes all the subsegments--the edges that are parts of input");
	ERR_PRINT_STRING("    segments.  If the -c switch is used, the output .poly file also");
	ERR_PRINT_STRING("    includes all of the edges on the convex hull.  Hence, the output .poly");
	ERR_PRINT_STRING("    file is useful for finding edges associated with input segments and for");
	ERR_PRINT_STRING("    setting boundary conditions in finite element simulations.  Moreover,");
	ERR_PRINT_STRING("    you will need the output .poly file if you plan to refine the output");
	ERR_PRINT_STRING("    mesh, and don't want segments to be missing in later triangulations.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  .area files:");
	ERR_PRINT_STRING("    First line:  <# of triangles>");
	ERR_PRINT_STRING("    Following lines:  <triangle #> <maximum area>");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    An .area file associates with each triangle a maximum area that is used");
	ERR_PRINT_STRING("    for mesh refinement.  As with other file formats, every triangle must");
	ERR_PRINT_STRING("    be represented, and the triangles must be numbered consecutively.  A");
	ERR_PRINT_STRING("    triangle may be left unconstrained by assigning it a negative maximum");
	ERR_PRINT_STRING("    area.\n");
	ERR_PRINT_STRING("  .edge files:");
	ERR_PRINT_STRING("    First line:  <# of edges> <# of boundary markers (0 or 1)>");
	ERR_PRINT_STRING("    Following lines:  <edge #> <endpoint> <endpoint> [boundary marker]");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    Endpoints are indices into the corresponding .node file.  Triangle can");
	ERR_PRINT_STRING("    produce .edge files (use the -e switch), but cannot read them.  The");
	ERR_PRINT_STRING("    optional column of boundary markers is suppressed by the -B switch.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    In Voronoi diagrams, one also finds a special kind of edge that is an");
	ERR_PRINT_STRING("    infinite ray with only one endpoint.  For these edges, a different");
	ERR_PRINT_STRING("    format is used:\n");
	ERR_PRINT_STRING("        <edge #> <endpoint> -1 <direction x> <direction y>\n");
	ERR_PRINT_STRING("    The `direction' is a floating-point vector that indicates the direction");
	ERR_PRINT_STRING("    of the infinite ray.\n");
	ERR_PRINT_STRING("  .neigh files:");
	ERR_PRINT_STRING("    First line:  <# of triangles> <# of neighbors per triangle (always 3)>");
	ERR_PRINT_STRING("    Following lines:  <triangle #> <neighbor> <neighbor> <neighbor>");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    Neighbors are indices into the corresponding .ele file.  An index of -1");
	ERR_PRINT_STRING("    indicates no neighbor (because the triangle is on an exterior");
	ERR_PRINT_STRING("    boundary).  The first neighbor of triangle i is opposite the first");
	ERR_PRINT_STRING("    corner of triangle i, and so on.\n");
	ERR_PRINT_STRING("    Triangle can produce .neigh files (use the -n switch), but cannot read");
	ERR_PRINT_STRING("    them.\n");
	ERR_PRINT_STRING("Boundary Markers:\n");
	ERR_PRINT_STRING("  Boundary markers are tags used mainly to identify which output vertices");
	ERR_PRINT_STRING("  and edges are associated with which PSLG segment, and to identify which");
	ERR_PRINT_STRING("  vertices and edges occur on a boundary of the triangulation.  A common");
	ERR_PRINT_STRING("  use is to determine where boundary conditions should be applied to a");
	ERR_PRINT_STRING("  finite element mesh.  You can prevent boundary markers from being written");
	ERR_PRINT_STRING("  into files produced by Triangle by using the -B switch.\n");
	ERR_PRINT_STRING("  The boundary marker associated with each segment in an output .poly file");
	ERR_PRINT_STRING("  and each edge in an output .edge file is chosen as follows:");
	ERR_PRINT_STRING("    - If an output edge is part or all of a PSLG segment with a nonzero");
	ERR_PRINT_STRING("      boundary marker, then the edge is assigned the same marker.");
	ERR_PRINT_STRING("    - Otherwise, if the edge lies on a boundary of the triangulation");
	ERR_PRINT_STRING("      (even the boundary of a hole), then the edge is assigned the marker");
	ERR_PRINT_STRING("      one (1).");
	ERR_PRINT_STRING("    - Otherwise, the edge is assigned the marker zero (0).");
	ERR_PRINT_STRING("  The boundary marker associated with each vertex in an output .node file");
	ERR_PRINT_STRING("  is chosen as follows:");
	ERR_PRINT_STRING("    - If a vertex is assigned a nonzero boundary marker in the input file,");
	ERR_PRINT_STRING("      then it is assigned the same marker in the output .node file.");
	ERR_PRINT_STRING("    - Otherwise, if the vertex lies on a PSLG segment (even if it is an");
	ERR_PRINT_STRING("      endpoint of the segment) with a nonzero boundary marker, then the");
	ERR_PRINT_STRING("      vertex is assigned the same marker.  If the vertex lies on several");
	ERR_PRINT_STRING("      such segments, one of the markers is chosen arbitrarily.");
	ERR_PRINT_STRING("    - Otherwise, if the vertex occurs on a boundary of the triangulation,");
	ERR_PRINT_STRING("      then the vertex is assigned the marker one (1).");
	ERR_PRINT_STRING("    - Otherwise, the vertex is assigned the marker zero (0).");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  If you want Triangle to determine for you which vertices and edges are on");
	ERR_PRINT_STRING("  the boundary, assign them the boundary marker zero (or use no markers at");
	ERR_PRINT_STRING("  all) in your input files.  In the output files, all boundary vertices,");
	ERR_PRINT_STRING("  edges, and segments will be assigned the value one.\n");
	ERR_PRINT_STRING("Triangulation Iteration Numbers:\n");
	ERR_PRINT_STRING("  Because Triangle can read and refine its own triangulations, input");
	ERR_PRINT_STRING("  and output files have iteration numbers.  For instance, Triangle might");
	ERR_PRINT_STRING("  read the files mesh.3.node, mesh.3.ele, and mesh.3.poly, refine the");
	ERR_PRINT_STRING("  triangulation, and output the files mesh.4.node, mesh.4.ele, and");
	ERR_PRINT_STRING("  mesh.4.poly.  Files with no iteration number are treated as if");
	ERR_PRINT_STRING("  their iteration number is zero; hence, Triangle might read the file");
	ERR_PRINT_STRING("  points.node, triangulate it, and produce the files points.1.node and");
	ERR_PRINT_STRING("  points.1.ele.\n");
	ERR_PRINT_STRING("  Iteration numbers allow you to create a sequence of successively finer");
	ERR_PRINT_STRING("  meshes suitable for multigrid methods.  They also allow you to produce a");
	ERR_PRINT_STRING("  sequence of meshes using error estimate-driven mesh refinement.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  If you're not using refinement or quality meshing, and you don't like");
	ERR_PRINT_STRING("  iteration numbers, use the -I switch to disable them.  This switch also");
	ERR_PRINT_STRING("  disables output of .node and .poly files to prevent your input files from");
	ERR_PRINT_STRING("  being overwritten.  (If the input is a .poly file that contains its own");
	ERR_PRINT_STRING("  points, a .node file is written.  This can be quite convenient for");
	ERR_PRINT_STRING("  computing CDTs or quality meshes.)\n");
	ERR_PRINT_STRING("Examples of How to Use Triangle:\n");
	ERR_PRINT_STRING("  `triangle dots' reads vertices from dots.node, and writes their Delaunay");
	ERR_PRINT_STRING("  triangulation to dots.1.node and dots.1.ele.  (dots.1.node is identical");
	ERR_PRINT_STRING("  to dots.node.)  `triangle -I dots' writes the triangulation to dots.ele");
	ERR_PRINT_STRING("  instead.  (No additional .node file is needed, so none is written.)");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  `triangle -pe object.1' reads a PSLG from object.1.poly (and possibly");
	ERR_PRINT_STRING("  object.1.node, if the vertices are omitted from object.1.poly) and writes");
	ERR_PRINT_STRING("  its constrained Delaunay triangulation to object.2.node and object.2.ele.");
	ERR_PRINT_STRING("  The segments are copied to object.2.poly, and all edges are written to");
	ERR_PRINT_STRING("  object.2.edge.\n");
	ERR_PRINT_STRING("  `triangle -pq31.5a.1 object' reads a PSLG from object.poly (and possibly");
	ERR_PRINT_STRING("  object.node), generates a mesh whose angles are all between 31.5 and 117");
	ERR_PRINT_STRING("  degrees and whose triangles all have areas of 0.1 or less, and writes the");
	ERR_PRINT_STRING("  mesh to object.1.node and object.1.ele.  Each segment may be broken up");
	ERR_PRINT_STRING("  into multiple subsegments; these are written to object.1.poly.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  Here is a sample file `box.poly' describing a square with a square hole:");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    # A box with eight vertices in 2D, no attributes, one boundary marker.");
	ERR_PRINT_STRING("    8 2 0 1");
	ERR_PRINT_STRING("     # Outer box has these vertices:");
	ERR_PRINT_STRING("     1   0 0   0");
	ERR_PRINT_STRING("     2   0 3   0");
	ERR_PRINT_STRING("     3   3 0   0");
	ERR_PRINT_STRING("     4   3 3   33     # A special marker for this vertex.");
	ERR_PRINT_STRING("     # Inner square has these vertices:");
	ERR_PRINT_STRING("     5   1 1   0");
	ERR_PRINT_STRING("     6   1 2   0");
	ERR_PRINT_STRING("     7   2 1   0");
	ERR_PRINT_STRING("     8   2 2   0");
	ERR_PRINT_STRING("    # Five segments with boundary markers.");
	ERR_PRINT_STRING("    5 1");
	ERR_PRINT_STRING("     1   1 2   5      # Left side of outer box.");
	ERR_PRINT_STRING("     # Square hole has these segments:");
	ERR_PRINT_STRING("     2   5 7   0");
	ERR_PRINT_STRING("     3   7 8   0");
	ERR_PRINT_STRING("     4   8 6   10");
	ERR_PRINT_STRING("     5   6 5   0");
	ERR_PRINT_STRING("    # One hole in the middle of the inner square.");
	ERR_PRINT_STRING("    1");
	ERR_PRINT_STRING("     1   1.5 1.5");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  Note that some segments are missing from the outer square, so you must");
	ERR_PRINT_STRING("  use the `-c' switch.  After `triangle -pqc box.poly', here is the output");
	ERR_PRINT_STRING("  file `box.1.node', with twelve vertices.  The last four vertices were");
	ERR_PRINT_STRING("  added to meet the angle constraint.  Vertices 1, 2, and 9 have markers");
	ERR_PRINT_STRING("  from segment 1.  Vertices 6 and 8 have markers from segment 4.  All the");
	ERR_PRINT_STRING("  other vertices but 4 have been marked to indicate that they lie on a");
	ERR_PRINT_STRING("  boundary.\n");
	ERR_PRINT_STRING("    12  2  0  1");
	ERR_PRINT_STRING("       1    0   0      5");
	ERR_PRINT_STRING("       2    0   3      5");
	ERR_PRINT_STRING("       3    3   0      1");
	ERR_PRINT_STRING("       4    3   3     33");
	ERR_PRINT_STRING("       5    1   1      1");
	ERR_PRINT_STRING("       6    1   2     10");
	ERR_PRINT_STRING("       7    2   1      1");
	ERR_PRINT_STRING("       8    2   2     10");
	ERR_PRINT_STRING("       9    0   1.5    5");
	ERR_PRINT_STRING("      10    1.5   0    1");
	ERR_PRINT_STRING("      11    3   1.5    1");
	ERR_PRINT_STRING("      12    1.5   3    1");
	ERR_PRINT_STRING("    # Generated by triangle -pqc box.poly");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  Here is the output file `box.1.ele', with twelve triangles.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    12  3  0");
	ERR_PRINT_STRING("       1     5   6   9");
	ERR_PRINT_STRING("       2    10   3   7");
	ERR_PRINT_STRING("       3     6   8  12");
	ERR_PRINT_STRING("       4     9   1   5");
	ERR_PRINT_STRING("       5     6   2   9");
	ERR_PRINT_STRING("       6     7   3  11");
	ERR_PRINT_STRING("       7    11   4   8");
	ERR_PRINT_STRING("       8     7   5  10");
	ERR_PRINT_STRING("       9    12   2   6");
	ERR_PRINT_STRING("      10     8   7  11");
	ERR_PRINT_STRING("      11     5   1  10");
	ERR_PRINT_STRING("      12     8   4  12");
	ERR_PRINT_STRING("    # Generated by triangle -pqc box.poly\n");
	ERR_PRINT_STRING("  Here is the output file `box.1.poly'.  Note that segments have been added");
	ERR_PRINT_STRING("  to represent the convex hull, and some segments have been subdivided by");
	ERR_PRINT_STRING("  newly added vertices.  Note also that <# of vertices> is set to zero to");
	ERR_PRINT_STRING("  indicate that the vertices should be read from the .node file.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    0  2  0  1");
	ERR_PRINT_STRING("    12  1");
	ERR_PRINT_STRING("       1     1   9     5");
	ERR_PRINT_STRING("       2     5   7     1");
	ERR_PRINT_STRING("       3     8   7     1");
	ERR_PRINT_STRING("       4     6   8    10");
	ERR_PRINT_STRING("       5     5   6     1");
	ERR_PRINT_STRING("       6     3  10     1");
	ERR_PRINT_STRING("       7     4  11     1");
	ERR_PRINT_STRING("       8     2  12     1");
	ERR_PRINT_STRING("       9     9   2     5");
	ERR_PRINT_STRING("      10    10   1     1");
	ERR_PRINT_STRING("      11    11   3     1");
	ERR_PRINT_STRING("      12    12   4     1");
	ERR_PRINT_STRING("    1");
	ERR_PRINT_STRING("       1   1.5 1.5");
	ERR_PRINT_STRING("    # Generated by triangle -pqc box.poly");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("Refinement and Area Constraints:");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  The -r switch causes a mesh (.node and .ele files) to be read and");
	ERR_PRINT_STRING("  refined.  If the -p switch is also used, a .poly file is read and used to");
	ERR_PRINT_STRING("  specify edges that are constrained and cannot be eliminated (although");
	ERR_PRINT_STRING("  they can be subdivided into smaller edges) by the refinement process.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  When you refine a mesh, you generally want to impose tighter constraints.");
	ERR_PRINT_STRING("  One way to accomplish this is to use -q with a larger angle, or -a");
	ERR_PRINT_STRING("  followed by a smaller area than you used to generate the mesh you are");
	ERR_PRINT_STRING("  refining.  Another way to do this is to create an .area file, which");
	ERR_PRINT_STRING("  specifies a maximum area for each triangle, and use the -a switch");
	ERR_PRINT_STRING("  (without a number following).  Each triangle's area constraint is applied");
	ERR_PRINT_STRING("  to that triangle.  Area constraints tend to diffuse as the mesh is");
	ERR_PRINT_STRING("  refined, so if there are large variations in area constraint between");
	ERR_PRINT_STRING("  adjacent triangles, you may not get the results you want.  In that case,");
	ERR_PRINT_STRING("  consider instead using the -u switch and writing a C procedure that");
	ERR_PRINT_STRING("  determines which triangles are too large.\n");
	ERR_PRINT_STRING("  If you are refining a mesh composed of linear (three-node) elements, the");
	ERR_PRINT_STRING("  output mesh contains all the nodes present in the input mesh, in the same");
	ERR_PRINT_STRING("  order, with new nodes added at the end of the .node file.  However, the");
	ERR_PRINT_STRING("  refinement is not hierarchical: there is no guarantee that each output");
	ERR_PRINT_STRING("  element is contained in a single input element.  Often, an output element");
	ERR_PRINT_STRING("  can overlap two or three input elements, and some input edges are not");
	ERR_PRINT_STRING("  present in the output mesh.  Hence, a sequence of refined meshes forms a");
	ERR_PRINT_STRING("  hierarchy of nodes, but not a hierarchy of elements.  If you refine a");
	ERR_PRINT_STRING("  mesh of higher-order elements, the hierarchical property applies only to");
	ERR_PRINT_STRING("  the nodes at the corners of an element; the midpoint nodes on each edge");
	ERR_PRINT_STRING("  are discarded before the mesh is refined.\n");
	ERR_PRINT_STRING("  Maximum area constraints in .poly files operate differently from those in");
	ERR_PRINT_STRING("  .area files.  A maximum area in a .poly file applies to the whole");
	ERR_PRINT_STRING("  (segment-bounded) region in which a point falls, whereas a maximum area");
	ERR_PRINT_STRING("  in an .area file applies to only one triangle.  Area constraints in .poly");
	ERR_PRINT_STRING("  files are used only when a mesh is first generated, whereas area");
	ERR_PRINT_STRING("  constraints in .area files are used only to refine an existing mesh, and");
	ERR_PRINT_STRING("  are typically based on a posteriori error estimates resulting from a");
	ERR_PRINT_STRING("  finite element simulation on that mesh.\n");
	ERR_PRINT_STRING("  `triangle -rq25 object.1' reads object.1.node and object.1.ele, then");
	ERR_PRINT_STRING("  refines the triangulation to enforce a 25 degree minimum angle, and then");
	ERR_PRINT_STRING("  writes the refined triangulation to object.2.node and object.2.ele.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  `triangle -rpaa6.2 z.3' reads z.3.node, z.3.ele, z.3.poly, and z.3.area.");
	ERR_PRINT_STRING("  After reconstructing the mesh and its subsegments, Triangle refines the");
	ERR_PRINT_STRING("  mesh so that no triangle has area greater than 6.2, and furthermore the");
	ERR_PRINT_STRING("  triangles satisfy the maximum area constraints in z.3.area.  No angle");
	ERR_PRINT_STRING("  bound is imposed at all.  The output is written to z.4.node, z.4.ele, and");
	ERR_PRINT_STRING("  z.4.poly.\n");
	ERR_PRINT_STRING("  The sequence `triangle -qa1 x', `triangle -rqa.3 x.1', `triangle -rqa.1");
	ERR_PRINT_STRING("  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,");
	ERR_PRINT_STRING("  suitable for multigrid.\n");
	ERR_PRINT_STRING("Convex Hulls and Mesh Boundaries:\n");
	ERR_PRINT_STRING("  If the input is a vertex set (not a PSLG), Triangle produces its convex");
	ERR_PRINT_STRING("  hull as a by-product in the output .poly file if you use the -c switch.");
	ERR_PRINT_STRING("  There are faster algorithms for finding a two-dimensional convex hull");
	ERR_PRINT_STRING("  than triangulation, of course, but this one comes for free.\n");
	ERR_PRINT_STRING("  If the input is an unconstrained mesh (you are using the -r switch but");
	ERR_PRINT_STRING("  not the -p switch), Triangle produces a list of its boundary edges");
	ERR_PRINT_STRING("  (including hole boundaries) as a by-product when you use the -c switch.");
	ERR_PRINT_STRING("  If you also use the -p switch, the output .poly file contains all the");
	ERR_PRINT_STRING("  segments from the input .poly file as well.\n");
	ERR_PRINT_STRING("Voronoi Diagrams:\n");
	ERR_PRINT_STRING("  The -v switch produces a Voronoi diagram, in files suffixed .v.node and");
	ERR_PRINT_STRING("  .v.edge.  For example, `triangle -v points' reads points.node, produces");
	ERR_PRINT_STRING("  its Delaunay triangulation in points.1.node and points.1.ele, and");
	ERR_PRINT_STRING("  produces its Voronoi diagram in points.1.v.node and points.1.v.edge.  The");
	ERR_PRINT_STRING("  .v.node file contains a list of all Voronoi vertices, and the .v.edge");
	ERR_PRINT_STRING("  file contains a list of all Voronoi edges, some of which may be infinite");
	ERR_PRINT_STRING("  rays.  (The choice of filenames makes it easy to run the set of Voronoi");
	ERR_PRINT_STRING("  vertices through Triangle, if so desired.)\n");
	ERR_PRINT_STRING("  This implementation does not use exact arithmetic to compute the Voronoi");
	ERR_PRINT_STRING("  vertices, and does not check whether neighboring vertices are identical.");
	ERR_PRINT_STRING("  Be forewarned that if the Delaunay triangulation is degenerate or");
	ERR_PRINT_STRING("  near-degenerate, the Voronoi diagram may have duplicate vertices or");
	ERR_PRINT_STRING("  crossing edges.\n");
	ERR_PRINT_STRING("  The result is a valid Voronoi diagram only if Triangle's output is a true");
	ERR_PRINT_STRING("  Delaunay triangulation.  The Voronoi output is usually meaningless (and");
	ERR_PRINT_STRING("  may contain crossing edges and other pathology) if the output is a CDT or");
	ERR_PRINT_STRING("  CCDT, or if it has holes or concavities.  If the triangulated domain is");
	ERR_PRINT_STRING("  convex and has no holes, you can use -D switch to force Triangle to");
	ERR_PRINT_STRING("  construct a conforming Delaunay triangulation instead of a CCDT, so the");
	ERR_PRINT_STRING("  Voronoi diagram will be valid.\n");
	ERR_PRINT_STRING("Mesh Topology:\n");
	ERR_PRINT_STRING("  You may wish to know which triangles are adjacent to a certain Delaunay");
	ERR_PRINT_STRING("  edge in an .edge file, which Voronoi cells are adjacent to a certain");
	ERR_PRINT_STRING("  Voronoi edge in a .v.edge file, or which Voronoi cells are adjacent to");
	ERR_PRINT_STRING("  each other.  All of this information can be found by cross-referencing");
	ERR_PRINT_STRING("  output files with the recollection that the Delaunay triangulation and");
	ERR_PRINT_STRING("  the Voronoi diagram are planar duals.\n");
	ERR_PRINT_STRING("  Specifically, edge i of an .edge file is the dual of Voronoi edge i of");
	ERR_PRINT_STRING("  the corresponding .v.edge file, and is rotated 90 degrees counterclock-");
	ERR_PRINT_STRING("  wise from the Voronoi edge.  Triangle j of an .ele file is the dual of");
	ERR_PRINT_STRING("  vertex j of the corresponding .v.node file.  Voronoi cell k is the dual");
	ERR_PRINT_STRING("  of vertex k of the corresponding .node file.\n");
	ERR_PRINT_STRING("  Hence, to find the triangles adjacent to a Delaunay edge, look at the");
	ERR_PRINT_STRING("  vertices of the corresponding Voronoi edge.  If the endpoints of a");
	ERR_PRINT_STRING("  Voronoi edge are Voronoi vertices 2 and 6 respectively, then triangles 2");
	ERR_PRINT_STRING("  and 6 adjoin the left and right sides of the corresponding Delaunay edge,");
	ERR_PRINT_STRING("  respectively.  To find the Voronoi cells adjacent to a Voronoi edge, look");
	ERR_PRINT_STRING("  at the endpoints of the corresponding Delaunay edge.  If the endpoints of");
	ERR_PRINT_STRING("  a Delaunay edge are input vertices 7 and 12, then Voronoi cells 7 and 12");
	ERR_PRINT_STRING("  adjoin the right and left sides of the corresponding Voronoi edge,");
	ERR_PRINT_STRING("  respectively.  To find which Voronoi cells are adjacent to each other,");
	ERR_PRINT_STRING("  just read the list of Delaunay edges.\n");
	ERR_PRINT_STRING("  Triangle does not write a list of the edges adjoining each Voronoi cell,");
	ERR_PRINT_STRING("  but you can reconstructed it straightforwardly.  For instance, to find");
	ERR_PRINT_STRING("  all the edges of Voronoi cell 1, search the output .edge file for every");
	ERR_PRINT_STRING("  edge that has input vertex 1 as an endpoint.  The corresponding dual");
	ERR_PRINT_STRING("  edges in the output .v.edge file form the boundary of Voronoi cell 1.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  For each Voronoi vertex, the .neigh file gives a list of the three");
	ERR_PRINT_STRING("  Voronoi vertices attached to it.  You might find this more convenient");
	ERR_PRINT_STRING("  than the .v.edge file.\n");
	ERR_PRINT_STRING("Quadratic Elements:\n");
	ERR_PRINT_STRING("  Triangle generates meshes with subparametric quadratic elements if the");
	ERR_PRINT_STRING("  -o2 switch is specified.  Quadratic elements have six nodes per element,");
	ERR_PRINT_STRING("  rather than three.  `Subparametric' means that the edges of the triangles");
	ERR_PRINT_STRING("  are always straight, so that subparametric quadratic elements are");
	ERR_PRINT_STRING("  geometrically identical to linear elements, even though they can be used");
	ERR_PRINT_STRING("  with quadratic interpolating functions.  The three extra nodes of an");
	ERR_PRINT_STRING("  element fall at the midpoints of the three edges, with the fourth, fifth,");
	ERR_PRINT_STRING("  and sixth nodes appearing opposite the first, second, and third corners");
	ERR_PRINT_STRING("  respectively.\n");
	ERR_PRINT_STRING("Domains with Small Angles:\n");
	ERR_PRINT_STRING("  If two input segments adjoin each other at a small angle, clearly the -q");
	ERR_PRINT_STRING("  switch cannot remove the small angle.  Moreover, Triangle may have no");
	ERR_PRINT_STRING("  choice but to generate additional triangles whose smallest angles are");
	ERR_PRINT_STRING("  smaller than the specified bound.  However, these triangles only appear");
	ERR_PRINT_STRING("  between input segments separated by small angles.  Moreover, if you");
	ERR_PRINT_STRING("  request a minimum angle of theta degrees, Triangle will generally produce");
	ERR_PRINT_STRING("  no angle larger than 180 - 2 theta, even if it is forced to compromise on");
	ERR_PRINT_STRING("  the minimum angle.\n");
	ERR_PRINT_STRING("Statistics:\n");
	ERR_PRINT_STRING("  After generating a mesh, Triangle prints a count of entities in the");
	ERR_PRINT_STRING("  output mesh, including the number of vertices, triangles, edges, exterior");
	ERR_PRINT_STRING("  boundary edges (i.e. subsegments on the boundary of the triangulation,");
	ERR_PRINT_STRING("  including hole boundaries), interior boundary edges (i.e. subsegments of");
	ERR_PRINT_STRING("  input segments not on the boundary), and total subsegments.  If you've");
	ERR_PRINT_STRING("  forgotten the statistics for an existing mesh, run Triangle on that mesh");
	ERR_PRINT_STRING("  with the -rNEP switches to read the mesh and print the statistics without");
	ERR_PRINT_STRING("  writing any files.  Use -rpNEP if you've got a .poly file for the mesh.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  The -V switch produces extended statistics, including a rough estimate");
	ERR_PRINT_STRING("  of memory use, the number of calls to geometric predicates, and");
	ERR_PRINT_STRING("  histograms of the angles and the aspect ratios of the triangles in the");
	ERR_PRINT_STRING("  mesh.\n");
	ERR_PRINT_STRING("Exact Arithmetic:\n");
	ERR_PRINT_STRING("  Triangle uses adaptive exact arithmetic to perform what computational");
	ERR_PRINT_STRING("  geometers call the `orientation' and `incircle' tests.  If the floating-");
	ERR_PRINT_STRING("  point arithmetic of your machine conforms to the IEEE 754 standard (as");
	ERR_PRINT_STRING("  most workstations do), and does not use extended precision internal");
	ERR_PRINT_STRING("  floating-point registers, then your output is guaranteed to be an");
	ERR_PRINT_STRING("  absolutely true Delaunay or constrained Delaunay triangulation, roundoff");
	ERR_PRINT_STRING("  error notwithstanding.  The word `adaptive' implies that these arithmetic");
	ERR_PRINT_STRING("  routines compute the result only to the precision necessary to guarantee");
	ERR_PRINT_STRING("  correctness, so they are usually nearly as fast as their approximate");
	ERR_PRINT_STRING("  counterparts.\n");
	ERR_PRINT_STRING("  May CPUs, including Intel x86 processors, have extended precision");
	ERR_PRINT_STRING("  floating-point registers.  These must be reconfigured so their precision");
	ERR_PRINT_STRING("  is reduced to memory precision.  Triangle does this if it is compiled");
	ERR_PRINT_STRING("  correctly.  See the makefile for details.\n");
	ERR_PRINT_STRING("  The exact tests can be disabled with the -X switch.  On most inputs, this");
	ERR_PRINT_STRING("  switch reduces the computation time by about eight percent--it's not");
	ERR_PRINT_STRING("  worth the risk.  There are rare difficult inputs (having many collinear");
	ERR_PRINT_STRING("  and cocircular vertices), however, for which the difference in speed");
	ERR_PRINT_STRING("  could be a factor of two.  Be forewarned that these are precisely the");
	ERR_PRINT_STRING("  inputs most likely to cause errors if you use the -X switch.  Hence, the");
	ERR_PRINT_STRING("  -X switch is not recommended.\n");
	ERR_PRINT_STRING("  Unfortunately, the exact tests don't solve every numerical problem.");
	ERR_PRINT_STRING("  Exact arithmetic is not used to compute the positions of new vertices,");
	ERR_PRINT_STRING("  because the bit complexity of vertex coordinates would grow without");
	ERR_PRINT_STRING("  bound.  Hence, segment intersections aren't computed exactly; in very");
	ERR_PRINT_STRING("  unusual cases, roundoff error in computing an intersection point might");
	ERR_PRINT_STRING("  actually lead to an inverted triangle and an invalid triangulation.");
	ERR_PRINT_STRING("  (This is one reason to specify your own intersection points in your .poly");
	ERR_PRINT_STRING("  files.)  Similarly, exact arithmetic is not used to compute the vertices");
	ERR_PRINT_STRING("  of the Voronoi diagram.\n");
	ERR_PRINT_STRING("  Another pair of problems not solved by the exact arithmetic routines is");
	ERR_PRINT_STRING("  underflow and overflow.  If Triangle is compiled for double precision");
	ERR_PRINT_STRING("  arithmetic, I believe that Triangle's geometric predicates work correctly");
	ERR_PRINT_STRING("  if the exponent of every input coordinate falls in the range [-148, 201].");
	ERR_PRINT_STRING("  Underflow can silently prevent the orientation and incircle tests from");
	ERR_PRINT_STRING("  being performed exactly, while overflow typically causes a floating");
	ERR_PRINT_STRING("  exception.\n");
	ERR_PRINT_STRING("Calling Triangle from Another Program:\n");
	ERR_PRINT_STRING("  Read the file triangle.h for details.\n");
	ERR_PRINT_STRING("Troubleshooting:\n");
	ERR_PRINT_STRING("  Please read this section before mailing me bugs.\n");
	ERR_PRINT_STRING("  `My output mesh has no triangles!'\n");
	ERR_PRINT_STRING("    If you're using a PSLG, you've probably failed to specify a proper set");
	ERR_PRINT_STRING("    of bounding segments, or forgotten to use the -c switch.  Or you may");
	ERR_PRINT_STRING("    have placed a hole badly, thereby eating all your triangles.  To test");
	ERR_PRINT_STRING("    these possibilities, try again with the -c and -O switches.");
	ERR_PRINT_STRING("    Alternatively, all your input vertices may be collinear, in which case");
	ERR_PRINT_STRING("    you can hardly expect to triangulate them.\n");
	ERR_PRINT_STRING("  `Triangle doesn't terminate, or just crashes.'\n");
	ERR_PRINT_STRING("    Bad things can happen when triangles get so small that the distance");
	ERR_PRINT_STRING("    between their vertices isn't much larger than the precision of your");
	ERR_PRINT_STRING("    machine's arithmetic.  If you've compiled Triangle for single-precision");
	ERR_PRINT_STRING("    arithmetic, you might do better by recompiling it for double-precision.");
	ERR_PRINT_STRING("    Then again, you might just have to settle for more lenient constraints");
	ERR_PRINT_STRING("    on the minimum angle and the maximum area than you had planned.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    You can minimize precision problems by ensuring that the origin lies");
	ERR_PRINT_STRING("    inside your vertex set, or even inside the densest part of your");
	ERR_PRINT_STRING("    mesh.  If you're triangulating an object whose x-coordinates all fall");
	ERR_PRINT_STRING("    between 6247133 and 6247134, you're not leaving much floating-point");
	ERR_PRINT_STRING("    precision for Triangle to work with.\n");
	ERR_PRINT_STRING("    Precision problems can occur covertly if the input PSLG contains two");
	ERR_PRINT_STRING("    segments that meet (or intersect) at an extremely small angle, or if");
	ERR_PRINT_STRING("    such an angle is introduced by the -c switch.  If you don't realize");
	ERR_PRINT_STRING("    that a tiny angle is being formed, you might never discover why");
	ERR_PRINT_STRING("    Triangle is crashing.  To check for this possibility, use the -S switch");
	ERR_PRINT_STRING("    (with an appropriate limit on the number of Steiner points, found by");
	ERR_PRINT_STRING("    trial-and-error) to stop Triangle early, and view the output .poly file");
	ERR_PRINT_STRING("    with Show Me (described below).  Look carefully for regions where dense");
	ERR_PRINT_STRING("    clusters of vertices are forming and for small angles between segments.");
	ERR_PRINT_STRING("    Zoom in closely, as such segments might look like a single segment from");
	ERR_PRINT_STRING("    a distance.\n");
	ERR_PRINT_STRING("    If some of the input values are too large, Triangle may suffer a");
	ERR_PRINT_STRING("    floating exception due to overflow when attempting to perform an");
	ERR_PRINT_STRING("    orientation or incircle test.  (Read the section on exact arithmetic");
	ERR_PRINT_STRING("    above.)  Again, I recommend compiling Triangle for double (rather");
	ERR_PRINT_STRING("    than single) precision arithmetic.\n");
	ERR_PRINT_STRING("    Unexpected problems can arise if you use quality meshing (-q, -a, or");
	ERR_PRINT_STRING("    -u) with an input that is not segment-bounded--that is, if your input");
	ERR_PRINT_STRING("    is a vertex set, or you're using the -c switch.  If the convex hull of");
	ERR_PRINT_STRING("    your input vertices has collinear vertices on its boundary, an input");
	ERR_PRINT_STRING("    vertex that you think lies on the convex hull might actually lie just");
	ERR_PRINT_STRING("    inside the convex hull.  If so, the vertex and the nearby convex hull");
	ERR_PRINT_STRING("    edge form an extremely thin triangle.  When Triangle tries to refine");
	ERR_PRINT_STRING("    the mesh to enforce angle and area constraints, Triangle might generate");
	ERR_PRINT_STRING("    extremely tiny triangles, or it might fail because of insufficient");
	ERR_PRINT_STRING("    floating-point precision.\n");
	ERR_PRINT_STRING("  `The numbering of the output vertices doesn't match the input vertices.'");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    You may have had duplicate input vertices, or you may have eaten some");
	ERR_PRINT_STRING("    of your input vertices with a hole, or by placing them outside the area");
	ERR_PRINT_STRING("    enclosed by segments.  In any case, you can solve the problem by not");
	ERR_PRINT_STRING("    using the -j switch.\n");
	ERR_PRINT_STRING("  `Triangle executes without incident, but when I look at the resulting");
	ERR_PRINT_STRING("  mesh, it has overlapping triangles or other geometric inconsistencies.'");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    If you select the -X switch, Triangle occasionally makes mistakes due");
	ERR_PRINT_STRING("    to floating-point roundoff error.  Although these errors are rare,");
	ERR_PRINT_STRING("    don't use the -X switch.  If you still have problems, please report the");
	ERR_PRINT_STRING("    bug.\n");
	ERR_PRINT_STRING("  `Triangle executes without incident, but when I look at the resulting");
	ERR_PRINT_STRING("  Voronoi diagram, it has overlapping edges or other geometric");
	ERR_PRINT_STRING("  inconsistencies.'");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    If your input is a PSLG (-p), you can only expect a meaningful Voronoi");
	ERR_PRINT_STRING("    diagram if the domain you are triangulating is convex and free of");
	ERR_PRINT_STRING("    holes, and you use the -D switch to construct a conforming Delaunay");
	ERR_PRINT_STRING("    triangulation (instead of a CDT or CCDT).\n");
	ERR_PRINT_STRING("  Strange things can happen if you've taken liberties with your PSLG.  Do");
	ERR_PRINT_STRING("  you have a vertex lying in the middle of a segment?  Triangle sometimes");
	ERR_PRINT_STRING("  copes poorly with that sort of thing.  Do you want to lay out a collinear");
	ERR_PRINT_STRING("  row of evenly spaced, segment-connected vertices?  Have you simply");
	ERR_PRINT_STRING("  defined one long segment connecting the leftmost vertex to the rightmost");
	ERR_PRINT_STRING("  vertex, and a bunch of vertices lying along it?  This method occasionally");
	ERR_PRINT_STRING("  works, especially with horizontal and vertical lines, but often it");
	ERR_PRINT_STRING("  doesn't, and you'll have to connect each adjacent pair of vertices with a");
	ERR_PRINT_STRING("  separate segment.  If you don't like it, tough.\n");
	ERR_PRINT_STRING("  Furthermore, if you have segments that intersect other than at their");
	ERR_PRINT_STRING("  endpoints, try not to let the intersections fall extremely close to PSLG");
	ERR_PRINT_STRING("  vertices or each other.\n");
	ERR_PRINT_STRING("  If you have problems refining a triangulation not produced by Triangle:");
	ERR_PRINT_STRING("  Are you sure the triangulation is geometrically valid?  Is it formatted");
	ERR_PRINT_STRING("  correctly for Triangle?  Are the triangles all listed so the first three");
	ERR_PRINT_STRING("  vertices are their corners in counterclockwise order?  Are all of the");
	ERR_PRINT_STRING("  triangles constrained Delaunay?  Triangle's Delaunay refinement algorithm");
	ERR_PRINT_STRING("  assumes that it starts with a CDT.\n");
	ERR_PRINT_STRING("Show Me:\n");
	ERR_PRINT_STRING("  Triangle comes with a separate program named `Show Me', whose primary");
	ERR_PRINT_STRING("  purpose is to draw meshes on your screen or in PostScript.  Its secondary");
	ERR_PRINT_STRING("  purpose is to check the validity of your input files, and do so more");
	ERR_PRINT_STRING("  thoroughly than Triangle does.  Unlike Triangle, Show Me requires that");
	ERR_PRINT_STRING("  you have the X Windows system.  Sorry, Microsoft Windows users.");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("Triangle on the Web:");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  To see an illustrated version of these instructions, check out");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("    http://www.cs.cmu.edu/~quake/triangle.html");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("A Brief Plea:");
	ERR_PRINT_STRING("");
	ERR_PRINT_STRING("  If you use Triangle, and especially if you use it to accomplish real");
	ERR_PRINT_STRING("  work, I would like very much to hear from you.  A short letter or email");
	ERR_PRINT_STRING("  (to jrs@cs.berkeley.edu) describing how you use Triangle will mean a lot");
	ERR_PRINT_STRING("  to me.  The more people I know are using this program, the more easily I");
	ERR_PRINT_STRING("  can justify spending time on improvements, which in turn will benefit");
	ERR_PRINT_STRING("  you.  Also, I can put you on a list to receive email whenever a new");
	ERR_PRINT_STRING("  version of Triangle is available.\n");
	ERR_PRINT_STRING("  If you use a mesh generated by Triangle in a publication, please include");
	ERR_PRINT_STRING("  an acknowledgment as well.  And please spell Triangle with a capital `T'!");
	ERR_PRINT_STRING("  If you want to include a citation, use `Jonathan Richard Shewchuk,");
	ERR_PRINT_STRING("  ``Triangle: Engineering a 2D Quality Mesh Generator and Delaunay");
	ERR_PRINT_STRING("  Triangulator,'' in Applied Computational Geometry:  Towards Geometric");
	ERR_PRINT_STRING("  Engineering (Ming C. Lin and Dinesh Manocha, editors), volume 1148 of");
	ERR_PRINT_STRING("  Lecture Notes in Computer Science, pages 203-222, Springer-Verlag,");
	ERR_PRINT_STRING("  Berlin, May 1996.  (From the First ACM Workshop on Applied Computational");
	ERR_PRINT_STRING("  Geometry.)'\n");
	ERR_PRINT_STRING("Research credit:\n");
	ERR_PRINT_STRING("  Of course, I can take credit for only a fraction of the ideas that made");
	ERR_PRINT_STRING("  this mesh generator possible.  Triangle owes its existence to the efforts");
	ERR_PRINT_STRING("  of many fine computational geometers and other researchers, including");
	ERR_PRINT_STRING("  Marshall Bern, L. Paul Chew, Kenneth L. Clarkson, Boris Delaunay, Rex A.");
	ERR_PRINT_STRING("  Dwyer, David Eppstein, Steven Fortune, Leonidas J. Guibas, Donald E.");
	ERR_PRINT_STRING("  Knuth, Charles L. Lawson, Der-Tsai Lee, Gary L. Miller, Ernst P. Mucke,");
	ERR_PRINT_STRING("  Steven E. Pav, Douglas M. Priest, Jim Ruppert, Isaac Saias, Bruce J.");
	ERR_PRINT_STRING("  Schachter, Micha Sharir, Peter W. Shor, Daniel D. Sleator, Jorge Stolfi,");
	ERR_PRINT_STRING("  Robert E. Tarjan, Alper Ungor, Christopher J. Van Wyk, Noel J.");
	ERR_PRINT_STRING("  Walkington, and Binhai Zhu.  See the comments at the beginning of the");
	ERR_PRINT_STRING("  source code for references.\n");
	triexit(0);
}
...
void	internalerror()
{
	ERR_PRINT_STRING("  Please report this bug to jrs@cs.berkeley.edu");
	ERR_PRINT_STRING("  Include the message above, your input data set, and the exact");
	ERR_PRINT_STRING("    command line you used to run Triangle.");
	triexit(1);
}
...
#define STARTINDEX 1
			...
			for (j = STARTINDEX; argv[i][j] != '\0'; ++j )
			{
				...
				if (argv[i][j] == 'q')
				{
					...
					if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) || (argv[i][j + 1] == '.'))
					{
						...
						workstring[k] = '\0';
						b->minangle = (double) strtod(workstring, (char **) nullptr);
					}
					else
					{
						b->minangle = 20.0;
				...
				if (argv[i][j] == 'a')
				{
					...
					if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) || (argv[i][j + 1] == '.'))
					{
						...
						workstring[k] = '\0';
						b->maxarea = (double) strtod(workstring, (char **) nullptr);
						if (b->maxarea <= 0.0)
						{
							ERR_PRINT_STRING("Error:  Maximum area must be greater than zero.");
							triexit(1);
						}
					}
					else
	...
	if (b->weighted && (b->poly || b->quality))
	{
		...
		if (!b->quiet)
		{
			ERR_PRINT_STRING("Warning:  weighted triangulations (-w, -W) are incompatible");
			ERR_PRINT_STRING("  with PSLGs (-p) and meshing (-q, -a, -u).  Weights ignored.");
		}
	}
	if (b->jettison && b->nonodewritten && !b->quiet)
	{
		ERR_PRINT_STRING("Warning:  -j and -N switches are somewhat incompatible.");
		ERR_PRINT_STRING("  If any vertices are jettisoned, you will need the output");
		ERR_PRINT_STRING("  .node file to reconstruct the new node indices.");
	}
...
#ifdef _MSC_VER
#	pragma warning( push )  
#	pragma warning (4 : 4311)	// disable pointer truncation warnings
#	pragma warning (4 : 4312)	// disable warnings
#	pragma warning (4 : 4302)	// disable truncation warnings
#endif	//_MSC_VER
	ERR_PRINT_STRING("triangle x%lx with orientation %d:", (unsigned long) t->tri, t->orient);
	decode(t->tri[0], printtri);
	if (printtri.tri == m->dummytri)
	{
		ERR_PRINT_STRING("    [0] = Outer space");
	}
	else
	{
		ERR_PRINT_STRING("    [0] = x%lx  %d", (unsigned long) printtri.tri,
		printtri.orient);
	}
	decode(t->tri[1], printtri);
	if (printtri.tri == m->dummytri)
	{
		ERR_PRINT_STRING("    [1] = Outer space");
	}
	else
	{
		ERR_PRINT_STRING("    [1] = x%lx  %d", (unsigned long) printtri.tri,
		printtri.orient);
	}
	decode(t->tri[2], printtri);
	if (printtri.tri == m->dummytri)
	{
		ERR_PRINT_STRING("    [2] = Outer space");
	}
	else
	{
		ERR_PRINT_STRING("    [2] = x%lx  %d", (unsigned long) printtri.tri,
		printtri.orient);
	}

	org(*t, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Origin[%d] = NULL", (t->orient + 1) % 3 + 3);
	else
		ERR_PRINT_STRING("    Origin[%d] = x%lx  (%.12g, %.12g)",
			(t->orient + 1) % 3 + 3, (unsigned long) printvertex,
			printvertex[0], printvertex[1]);
	dest(*t, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Dest  [%d] = NULL", (t->orient + 2) % 3 + 3);
	else
		ERR_PRINT_STRING("    Dest  [%d] = x%lx  (%.12g, %.12g)",
				(t->orient + 2) % 3 + 3, (unsigned long) printvertex,
				printvertex[0], printvertex[1]);
	apex(*t, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Apex  [%d] = NULL", t->orient + 3);
	else
		ERR_PRINT_STRING("    Apex  [%d] = x%lx  (%.12g, %.12g)",
	...
	if (b->usesegments)
	{
		...
		{
			ERR_PRINT_STRING("    [6] = x%lx  %d", (unsigned long) printsh.ss,
			printsh.ssorient);
		}
		sdecode(t->tri[7], printsh);
		if (printsh.ss != m->dummysub)
		{
			ERR_PRINT_STRING("    [7] = x%lx  %d", (unsigned long) printsh.ss,
			printsh.ssorient);
		}
		sdecode(t->tri[8], printsh);
		if (printsh.ss != m->dummysub)
		{
			ERR_PRINT_STRING("    [8] = x%lx  %d", (unsigned long) printsh.ss,
					printsh.ssorient);
		}
	}
	...
	if (b->vararea)
	{
		ERR_PRINT_STRING("    Area constraint:  %.4g", areabound(*t));
	}
}
...

{
	...
	ERR_PRINT_STRING("subsegment x%lx with orientation %d and mark %d:",
			(unsigned long) s->ss, s->ssorient, mark(*s));
			sdecode(s->ss[0], printsh);
	if (printsh.ss == m->dummysub)
	{
		ERR_PRINT_STRING("    [0] = No subsegment");
	}
	else
	{
		ERR_PRINT_STRING("    [0] = x%lx  %d", (unsigned long) printsh.ss,
		printsh.ssorient);
	}
	sdecode(s->ss[1], printsh);
	if (printsh.ss == m->dummysub)
	{
		ERR_PRINT_STRING("    [1] = No subsegment");
	}
	else
	{
		ERR_PRINT_STRING("    [1] = x%lx  %d", (unsigned long) printsh.ss,
		printsh.ssorient);
	}

	sorg(*s, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Origin[%d] = NULL", 2 + s->ssorient);
	else
		ERR_PRINT_STRING("    Origin[%d] = x%lx  (%.12g, %.12g)",
				...
				sdest(*s, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Dest  [%d] = NULL", 3 - s->ssorient);
	else
		ERR_PRINT_STRING("    Dest  [%d] = x%lx  (%.12g, %.12g)",
	...
	if (printtri.tri == m->dummytri)
	{
		ERR_PRINT_STRING("    [6] = Outer space");
	}
	else
	{
		ERR_PRINT_STRING("    [6] = x%lx  %d", (unsigned long) printtri.tri,
		printtri.orient);
	}
	decode(s->ss[7], printtri);
	if (printtri.tri == m->dummytri)
	{
		ERR_PRINT_STRING("    [7] = Outer space");
	}
	else
	{
		ERR_PRINT_STRING("    [7] = x%lx  %d", (unsigned long) printtri.tri,
		printtri.orient);
	}

	segorg(*s, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Segment origin[%d] = NULL", 4 + s->ssorient);
	else
		ERR_PRINT_STRING("    Segment origin[%d] = x%lx  (%.12g, %.12g)",
				4 + s->ssorient, (unsigned long) printvertex,
				printvertex[0], printvertex[1]);
				segdest(*s, printvertex);
	if (printvertex == (vertex) nullptr)
		ERR_PRINT_STRING("    Segment dest  [%d] = NULL", 5 - s->ssorient);
	else
		ERR_PRINT_STRING("    Segment dest  [%d] = x%lx  (%.12g, %.12g)",
...
{
	pool->firstblock = (VOID **) nullptr;
	pool->nowblock = (VOID **) nullptr;
	pool->nextitem = (VOID *) nullptr;
	pool->deaditemstack = (VOID *) nullptr;
	pool->pathblock = (VOID **) nullptr;
	pool->pathitem = (VOID *) nullptr;
	pool->alignbytes = 0;
	pool->itembytes = 0;
...
#endif /* not ANSI_DECLARATORS */
{
	while (pool->firstblock != (VOID **) nullptr)
	{
...
#endif /* not ANSI_DECLARATORS */
{
	...
	if (pool->deaditemstack != (VOID *) nullptr)
	{
...
#endif /* not ANSI_DECLARATORS */
{
	...
	/* Three NULL vertices. */
	m->dummytri[3] = (triangle) nullptr;
	m->dummytri[4] = (triangle) nullptr;
	m->dummytri[5] = (triangle) nullptr;
	...
	if (b->usesegments)
	{
		...
		/* Four NULL vertices. */
		m->dummysub[2] = (subseg) nullptr;
		m->dummysub[3] = (subseg) nullptr;
		m->dummysub[4] = (subseg) nullptr;
		m->dummysub[5] = (subseg) nullptr;
...
#endif /* not ANSI_DECLARATORS */
{
	...
	do
	{
		...
		if (newtriangle == (triangle *) nullptr)
		{
...
#endif /* not ANSI_DECLARATORS */
{
	...
	do
	{
		...
		if (newsubseg == (subseg *) nullptr)
		{
			return (subseg *) nullptr;
		}
	}
...
#endif /* not ANSI_DECLARATORS */
{
	...
	do
	{
		...
		if (newseg == (struct badsubseg *) nullptr)
		{
			return (struct badsubseg *) nullptr;
		}
	}
	while (newseg->subsegorg == (vertex) nullptr);           /* Skip dead ones. */
	return newseg;
}
...
#endif /* not ANSI_DECLARATORS */
{
	...
	/* Three NULL vertices. */
	newotri->tri[3] = (triangle) nullptr;
	newotri->tri[4] = (triangle) nullptr;
	newotri->tri[5] = (triangle) nullptr;
	if (b->usesegments)
	{
...
#endif /* not ANSI_DECLARATORS */
{
	...
	/* Four NULL vertices. */
	newsubseg->ss[2] = (subseg) nullptr;
	newsubseg->ss[3] = (subseg) nullptr;
	newsubseg->ss[4] = (subseg) nullptr;
	newsubseg->ss[5] = (subseg) nullptr;
	/* Initialize the two adjoining triangles to be "outer space." */
...

{
  ...
  m->recenttri.tri = (triangle *) nullptr; /* No triangle has been visited yet. */
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
	ERR_PRINT_STRING("  Checking consistency of mesh...");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle *) nullptr) {
	...
	for (triangleloop.orient = 0; triangleloop.orient < 3;
	  ...
	  if (oppotri.tri != m->dummytri) {
		...
		if ((triangleloop.tri != oppooppotri.tri)
		  ...
		  ERR_PRINT_STRING("  !! !! Asymmetric triangle-triangle bond:");
		  if (triangleloop.tri == oppooppotri.tri) {
			ERR_PRINT_STRING("   (Right triangle, wrong orientation)");
		  }
		  ERR_PRINT_STRING("    First ");
		  printtriangle(m, b, &triangleloop);
		  ERR_PRINT_STRING("    Second (nonreciprocating) ");
		...
		if ((triorg != oppodest) || (tridest != oppoorg)) {
		  ERR_PRINT_STRING("  !! !! Mismatched edge coordinates between two triangles:"
				 );
		  ERR_PRINT_STRING("    First mismatched ");
		  printtriangle(m, b, &triangleloop);
		  ERR_PRINT_STRING("    Second mismatched ");
		  printtriangle(m, b, &oppotri);
		  horrors++;
		}
	  }
  ...
  if (horrors == 0) {
	if (!b->quiet) {
	  ERR_PRINT_STRING("  In my studied opinion, the mesh appears to be consistent.");
	}
  } else if (horrors == 1) {
	ERR_PRINT_STRING("  !! !! !! !! Precisely one festering wound discovered.");
  } else {
	ERR_PRINT_STRING("  !! !! !! !! %d abominations witnessed.", horrors);
  }
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
	ERR_PRINT_STRING("  Checking Delaunay property of mesh...");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&m->triangles);
	...
	for (triangleloop.orient = 0; triangleloop.orient < 3;
	  ...
	  if (shouldbedelaunay) {
		if (nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0) {
		  if (!b->weighted) {
			ERR_PRINT_STRING("  !! !! Non-Delaunay pair of triangles:");
			ERR_PRINT_STRING("    First non-Delaunay ");
			printtriangle(m, b, &triangleloop);
			ERR_PRINT_STRING("    Second non-Delaunay ");
		  } else {
			ERR_PRINT_STRING("  !! !! Non-regular pair of triangles:" );
			ERR_PRINT_STRING("    First non-regular ");
			printtriangle(m, b, &triangleloop);
			ERR_PRINT_STRING("    Second non-regular ");
		  }
		  printtriangle(m, b, &oppotri);
		  horrors++;
		}
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay." );
	...
	ERR_PRINT_STRING("  !! !! !! !! %d obscenities viewed with horror." , horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Queueing bad triangle:" );
	ERR_PRINT_STRING("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)" ,
  ...
  if (m->queuefront[queuenumber] == (struct badtriang *) nullptr) {
	...
	if (queuenumber > m->firstnonemptyq) {
	  ...
	  i = queuenumber + 1;
	  while (m->queuefront[i] == (struct badtriang *) nullptr) {
		i++;
	  }
  ...
  badtri->nexttriang = (struct badtriang *) nullptr;
}
...

{
  ...
  if (m->firstnonemptyq < 0) {
	return (struct badtriang *) nullptr;
  }
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("    Constructing mapping from vertices to triangles." );
  }
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle *) nullptr) {
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Searching for point (%.12g, %.12g)." ,
  ...
  while (1) {
	if (b->verbose > 2) {
	  ERR_PRINT_STRING("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)",
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Randomly sampling for a triangle near point (%.12g, %.12g).",
		   searchpoint[0], searchpoint[1]);
  }
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("    Boundary triangle has origin (%.12g, %.12g).",
		   torg[0], torg[1]);
  }

  /* If a recently encountered triangle has been recorded and has not been */
  /*   deallocated, test it as a good starting point.                      */
  if (m->recenttri.tri != (triangle *) nullptr) {
	if (!deadtri(m->recenttri.tri)) {
	  ...
	  if (dist < searchdist) {
		...
		searchdist = dist;
		if (b->verbose > 2) {
		  ERR_PRINT_STRING("    Choosing recent triangle with origin (%.12g, %.12g).",
				 torg[0], torg[1]);
		}
	  }
	}
	...
	do {
	  ...
	  if (!deadtri(sampletri.tri)) {
		...
		if (dist < searchdist) {
		  ...
		  searchdist = dist;
		  if (b->verbose > 2) {
			ERR_PRINT_STRING("    Choosing triangle with origin (%.12g, %.12g).",
				   torg[0], torg[1]);
		  }
		}
	  }
...

{
  ...
  if (newsubseg.ss == m->dummysub) {
	...
	if (b->verbose > 2) {
	  ERR_PRINT_STRING("  Inserting new ");
...

{
  ...
  if (top.tri == m->dummytri) {
	ERR_PRINT_STRING("Internal error in flip():  Attempt to flip on boundary.");
	lnextself(*flipedge);
	return;
  }
  if (m->checksegments) {
	...
	if (toplsubseg.ss != m->dummysub) {
	  ERR_PRINT_STRING("Internal error in flip():  Attempt to flip a segment.");
	  lnextself(*flipedge);
	  return;
	}
  }
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Edge flip results in left ");
	printtriangle(m, b, &top);
	ERR_PRINT_STRING("  and right ");
...

{
...
#ifdef SELF_CHECK
  if (top.tri == m->dummytri) {
	ERR_PRINT_STRING("Internal error in unflip():  Attempt to flip on boundary.");
	lnextself(*flipedge);
	return;
  }
  if (m->checksegments) {
	tspivot(*flipedge, toplsubseg);
	if (toplsubseg.ss != m->dummysub) {
	  ERR_PRINT_STRING("Internal error in unflip():  Attempt to flip a subsegment.");
	  lnextself(*flipedge);
	  return;
	}
  }
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Edge unflip results in left ");
	printtriangle(m, b, flipedge);
	ERR_PRINT_STRING("  and right ");
	printtriangle(m, b, &top);
  }
...

{
  ...
  if (b->verbose > 1) {
	ERR_PRINT_STRING("  Inserting (%.12g, %.12g).", newvertex[0], newvertex[1]);
  }

  if (splitseg == (struct osub *) nullptr) {
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
	...
	if (m->checksegments && (splitseg == (struct osub *) nullptr)) {
	...
	if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
	  ERR_PRINT_STRING("Internal error in insertvertex():");
	  ERR_PRINT_STRING(
	...
	if (mirrorflag) {
	  if (counterclockwise(m, b, leftvertex, rightvertex, topvertex) < 0.0) {
		ERR_PRINT_STRING("Internal error in insertvertex():");
		ERR_PRINT_STRING("  Clockwise triangle prior to edge vertex insertion (top).");
	  }
...
#endif /* SELF_CHECK */
	if (b->verbose > 2) {
	  ERR_PRINT_STRING("  Updating bottom left ");
	  printtriangle(m, b, &botright);
	  if (mirrorflag) {
		ERR_PRINT_STRING("  Updating top left ");
		printtriangle(m, b, &topright);
		ERR_PRINT_STRING("  Creating top right ");
		printtriangle(m, b, &newtopright);
	  }
	  ERR_PRINT_STRING("  Creating bottom right ");
	...
	if (m->checkquality) {
	  ...
	  m->lastflip = (struct flipstacker *) poolalloc(&m->flipstackers);
	  m->lastflip->flippedtri = encode(horiz);
	  m->lastflip->prevflip = (struct flipstacker *) nullptr;
	}

#ifdef SELF_CHECK
	if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
	  ERR_PRINT_STRING("Internal error in insertvertex():");
	  ERR_PRINT_STRING("  Clockwise triangle prior to vertex insertion.");
	}
	if (counterclockwise(m, b, rightvertex, leftvertex, newvertex) < 0.0) {
	  ERR_PRINT_STRING("Internal error in insertvertex():");
	  ERR_PRINT_STRING("  Clockwise triangle after vertex insertion (top).");
	}
	if (counterclockwise(m, b, leftvertex, botvertex, newvertex) < 0.0) {
	  ERR_PRINT_STRING("Internal error in insertvertex():");
	  ERR_PRINT_STRING("  Clockwise triangle after vertex insertion (left).");
	}
	if (counterclockwise(m, b, botvertex, rightvertex, newvertex) < 0.0) {
	  ERR_PRINT_STRING("Internal error in insertvertex():");
	  ERR_PRINT_STRING("  Clockwise triangle after vertex insertion (right).");
	}
#endif /* SELF_CHECK */
	if (b->verbose > 2) {
	  ERR_PRINT_STRING("  Updating top ");
	  printtriangle(m, b, &horiz);
	  ERR_PRINT_STRING("  Creating left ");
	  printtriangle(m, b, &newbotleft);
	  ERR_PRINT_STRING("  Creating right ");
	  printtriangle(m, b, &newbotright);
	}
  }
		...
		if (doflip) {
		  ...
		  if (newvertex != (vertex) nullptr) {
			if (counterclockwise(m, b, leftvertex, newvertex, rightvertex) <
				0.0) {
			  ERR_PRINT_STRING("Internal error in insertvertex():");
			  ERR_PRINT_STRING("  Clockwise triangle prior to edge flip (bottom).");
			}
...
/*
			if (counterclockwise(m, b, rightvertex, farvertex, leftvertex) <
				0.0) {
			  ERR_PRINT_STRING("Internal error in insertvertex():");
			  ERR_PRINT_STRING("  Clockwise triangle prior to edge flip (top).");
			}
*/
			if (counterclockwise(m, b, farvertex, leftvertex, newvertex) <
				0.0) {
			  ERR_PRINT_STRING("Internal error in insertvertex():");
			  ERR_PRINT_STRING("  Clockwise triangle after edge flip (left).");
			}
			if (counterclockwise(m, b, newvertex, rightvertex, farvertex) <
				0.0) {
			  ERR_PRINT_STRING("Internal error in insertvertex():");
			  ERR_PRINT_STRING("  Clockwise triangle after edge flip (right).");
			}
		  }
#endif /* SELF_CHECK */
		  if (b->verbose > 2) {
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Triangulating interior polygon at edge");
	ERR_PRINT_STRING("    (%.12g, %.12g) (%.12g, %.12g)", leftbasevertex[0],
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("    Connecting edge to (%.12g, %.12g)", bestvertex[0],
		   bestvertex[1]);
  }
  if (bestnumber > 1) {
...

{
  ...
  if (b->verbose > 1) {
	ERR_PRINT_STRING("  Deleting (%.12g, %.12g).", delvertex[0], delvertex[1]);
  }
  ...
  while (!otriequal(*deltri, countingtri)) {
	...
	if (countingtri.tri == m->dummytri) {
	  ERR_PRINT_STRING("Internal error in deletevertex():");
	  ERR_PRINT_STRING("  Attempt to delete boundary vertex.");
	  internalerror();
	}
#endif /* SELF_CHECK */
	edgecount++;
...

{
  ...
  while (m->lastflip != (struct flipstacker *) nullptr) {
	...
	if (m->lastflip->prevflip == (struct flipstacker *) nullptr) {
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Creating base bounding ");
  ...
  while (1) {
	...
	if (leftfinished && rightfinished) {
	  ...
	  if (b->verbose > 2) {
		ERR_PRINT_STRING("  Creating top bounding ");
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("  Triangulating %d vertices.", vertices);
  }
  if (vertices == 2) {
	...
	divconqrecurse(m, b, &sortarray[divider], vertices - divider, 1 - axis,
				   &innerright, farright);
	if (b->verbose > 1) {
	  ERR_PRINT_STRING("  Joining triangulations with %d and %d vertices.", divider,
			 vertices - divider);
	}
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Removing ghost triangles.");
  }
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Sorting vertices.");
  }
  ...
  for (j = 1; j < m->invertices; ++j ) {
	if ((sortarray[i][0] == sortarray[j][0])
	  ...
	  if (!b->quiet) {
		ERR_PRINT_STRING("Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.",
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Forming triangulation.");
  }
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Creating triangular bounding box.");
  }
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Removing triangular bounding box.");
  }
  /* Find a boundary triangle. */
...

{
  ...
  boundingbox(m, b);
  if (b->verbose) {
	ERR_PRINT_STRING("  Incrementally inserting vertices.");
  }
  traversalinit(&m->vertices);
  vertexloop = vertextraverse(m);
  while (vertexloop != (vertex) nullptr) {
	starttri.tri = m->dummytri;
	if (insertvertex(m, b, vertexloop, &starttri, (struct osub *) nullptr, 0, 0)
		== DUPLICATEVERTEX) {
	  if (!b->quiet) {
		ERR_PRINT_STRING("Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.",
...

{
  ...
  *freeevents = (struct event *) nullptr;
...

{
  ...
  if (splaytree == (struct splaynode *) nullptr) {
	return (struct splaynode *) nullptr;
  }
  ...
  if (checkvertex == splaytree->keydest) {
	...
	if (child == (struct splaynode *) nullptr) {
	  return splaytree;
	}
	...
	if (checkvertex != child->keydest) {
	  ...
	  if (child == (struct splaynode *) nullptr) {
		if (rightofroot) {
		  splaytree->rchild = (struct splaynode *) nullptr;
		} else {
		  splaytree->lchild = (struct splaynode *) nullptr;
		}
		return splaytree;
	  }
	}
	...
	if (grandchild == (struct splaynode *) nullptr) {
	  if (rightofroot) {
	...
	if (lefttree == (struct splaynode *) nullptr) {
	  return righttree;
	} else if (righttree == (struct splaynode *) nullptr) {
	  return lefttree;
	} else if (lefttree->rchild == (struct splaynode *) nullptr) {
	  lefttree->rchild = righttree->lchild;
	  righttree->lchild = lefttree;
	  return righttree;
	} else if (righttree->lchild == (struct splaynode *) nullptr) {
	  righttree->lchild = lefttree->rchild;
	  lefttree->rchild = righttree;
	  return lefttree;
	} else {
/*      ERR_PRINT_STRING("Holy Toledo!!!"); */
	  leftright = lefttree->rchild;
	  while (leftright->rchild != (struct splaynode *) nullptr) {
...

{
  ...
  if (splayroot == (struct splaynode *) nullptr) {
	newsplaynode->lchild = (struct splaynode *) nullptr;
	newsplaynode->rchild = (struct splaynode *) nullptr;
  } else if (rightofhyperbola(m, &splayroot->keyedge, searchpoint)) {
	newsplaynode->lchild = splayroot;
	newsplaynode->rchild = splayroot->rchild;
	splayroot->rchild = (struct splaynode *) nullptr;
  } else {
	newsplaynode->lchild = splayroot->lchild;
	newsplaynode->rchild = splayroot;
	splayroot->lchild = (struct splaynode *) nullptr;
  }
  return newsplaynode;
}
...

{
  ...
  poolinit(&m->splaynodes, sizeof(struct splaynode), SPLAYNODEPERBLOCK,
		   SPLAYNODEPERBLOCK, 0);
  splayroot = (struct splaynode *) nullptr;

  if (b->verbose) {
	ERR_PRINT_STRING("  Placing vertices in event heap.");
  }
  ...
  do {
	if (heapsize == 0) {
	  ERR_PRINT_STRING("Error:  Input vertices are all identical.");
	  triexit(1);
	}
...
#else /* not REDUCED */
  if (!b->quiet) {
	ERR_PRINT_STRING("Constructing Delaunay triangulation ");
	if (b->incremental) {
	  ERR_PRINT_STRING("by incremental method.");
	} else if (b->sweepline) {
	  ERR_PRINT_STRING("by sweepline method.");
	} else {
	  ERR_PRINT_STRING("by divide-and-conquer method.");
	}
  }
  if (b->incremental) {
...

{
  ...
  incorners = corners;
  if (incorners < 3) {
	ERR_PRINT_STRING("Error:  Triangles must have at least 3 vertices.");
	triexit(1);
  }
  m->eextras = attribs;
#else /* not TRILIBRARY */
  ...
  if (!b->quiet) {
	ERR_PRINT_STRING("Opening %s.", elefilename);
  }
  elefile = fopen(elefilename, "r");
  if (elefile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot access file %s.", elefilename);
	triexit(1);
  }
  ...
  if (*stringptr == '\0') {
	...
	if (incorners < 3) {
	  ERR_PRINT_STRING("Error:  Triangles in %s must have at least 3 vertices.",
			 elefilename);
	  triexit(1);
	}
  }
  ...
  if (b->poly) {
	...
	segmentmarkers = segmentmarkerlist != (int *) nullptr;
#else /* not TRILIBRARY */
  ...
  if (b->vararea) {
	...
	if (!b->quiet) {
	  ERR_PRINT_STRING("Opening %s.", areafilename);
	}
	areafile = fopen(areafilename, "r");
	if (areafile == (FILE *) nullptr) {
	  ERR_PRINT_STRING("  Error:  Cannot access file %s.", areafilename);
	  triexit(1);
	}
	...
	if (areaelements != m->inelements) {
	  ERR_PRINT_STRING("Error:  %s and %s disagree on number of triangles.",
  ...
  if (!b->quiet) {
	ERR_PRINT_STRING("Reconstructing mesh.");
  }
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Assembling triangles.");
  }
  ...
  while (triangleloop.tri != (triangle *) nullptr) {
	...
	for (j = 0; j < 3; ++j ) {
	  ...
	  if ((corner[j] < b->firstnumber) ||
		...
		ERR_PRINT_STRING("Error:  Triangle %ld has an invalid vertex index.",
			   elementnumber);
		triexit(1);
	  }
	}
	...
	for (j = 0; j < 3; ++j ) {
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Triangle %ld is missing vertex %d in %s.",
  ...
  if (b->poly) {
	if (b->verbose) {
	  ERR_PRINT_STRING("  Marking segments in triangulation.");
	}
	...
	while (subsegloop.ss != (subseg *) nullptr) {
#ifdef TRILIBRARY
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Segment %ld has no endpoints in %s.", segmentnumber,
			   polyfilename);
		triexit(1);
	  } else {
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Segment %ld is missing its second endpoint in %s.",
	  ...
	  for (j = 0; j < 2; ++j ) {
		if ((end[j] < b->firstnumber) ||
		  ...
		  ERR_PRINT_STRING("Error:  Segment %ld has an invalid vertex index.", 
				 segmentnumber);
		  triexit(1);
		}
	  }
...

{
  ...
  while (leftflag) {
	...
	onextself(*searchtri);
	if (searchtri->tri == m->dummytri) {
	  ERR_PRINT_STRING("Internal error in finddirection():  Unable to find a");
	  ERR_PRINT_STRING("  triangle leading from (%.12g, %.12g) to", startvertex[0],
			 startvertex[1]);
	  ERR_PRINT_STRING("  (%.12g, %.12g).", searchpoint[0], searchpoint[1]);
	  internalerror();
	}
	apex(*searchtri, leftvertex);
	rightccw = leftccw;
...

{
  ...
  denom = ty * ex - tx * ey;
  if (denom == 0.0) {
	ERR_PRINT_STRING("Internal error in segmentintersection():");
	ERR_PRINT_STRING("  Attempt to find intersection of parallel segments.");
	internalerror();
  }
  ...
  if (success != SUCCESSFULVERTEX) {
	ERR_PRINT_STRING("Internal error in segmentintersection():");
	ERR_PRINT_STRING("  Failure to split a segment.");
	internalerror();
  }
  ...
  if ((leftvertex[0] == endpoint1[0]) && (leftvertex[1] == endpoint1[1])) {
	...
	ERR_PRINT_STRING("Internal error in segmentintersection():");
	ERR_PRINT_STRING("  Topological inconsistency after splitting a segment.");
	internalerror();
  }
...

{
  ...
  if (b->verbose > 2) {
	ERR_PRINT_STRING("Forcing segment into triangulation by recursive splitting:");
	ERR_PRINT_STRING("  (%.12g, %.12g) (%.12g, %.12g)", endpoint1[0], endpoint1[1],
  ...
  success = insertvertex(m, b, newvertex, &searchtri1, (struct osub *) nullptr,
						 0, 0);
  if (success == DUPLICATEVERTEX) {
	if (b->verbose > 2) {
	  ERR_PRINT_STRING("  Segment intersects existing vertex (%.12g, %.12g).",
	...
	if (success == VIOLATINGVERTEX) {
	  if (b->verbose > 2) {
		ERR_PRINT_STRING("  Two segments intersect at (%.12g, %.12g).",
			   newvertex[0], newvertex[1]);
	  }
	  ...
	  success = insertvertex(m, b, newvertex, &searchtri1, &brokensubseg,
							 0, 0);
	  if (success != SUCCESSFULVERTEX) {
		ERR_PRINT_STRING("Internal error in conformingedge():");
		ERR_PRINT_STRING("  Failure to split a segment.");
		internalerror();
	  }
	}
...

{
  ...
  if (b->verbose > 1) {
	ERR_PRINT_STRING("  Connecting (%.12g, %.12g) to (%.12g, %.12g).",
		   endpoint1[0], endpoint1[1], endpoint2[0], endpoint2[1]);
  }

  /* Find a triangle whose origin is the segment's first endpoint. */
  checkvertex = (vertex) nullptr;
  encodedtri = vertex2tri(endpoint1);
  if (encodedtri != (triangle) nullptr) {
  ...
  if (checkvertex != endpoint1) {
	...
	if (locate(m, b, endpoint1, &searchtri1) != ONVERTEX) {
	  ...
	  ERR_PRINT_STRING("  (%.12g, %.12g) in triangulation.",
...

{
  ...
  if (b->poly) {
	if (!b->quiet) {
	  ERR_PRINT_STRING("Recovering segments in Delaunay triangulation.");
	}
#ifdef TRILIBRARY
	...
	if (m->insegments > 0) {
	  makevertexmap(m, b);
	  if (b->verbose) {
		ERR_PRINT_STRING("  Recovering PSLG segments.");
	  }
	}
	...
	for (i = 0; i < m->insegments; ++i ) {
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Segment %d has no endpoints in %s.",
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Segment %d is missing its second endpoint in %s.",
			   b->firstnumber + i, polyfilename);
		triexit(1);
	  } else {
	  ...
	  if ((end1 < b->firstnumber) ||
		...
		if (!b->quiet) {
		  ERR_PRINT_STRING("Warning:  Invalid first endpoint of segment %d in %s.",
				 b->firstnumber + i, polyfilename);
		}
	  } else if ((end2 < b->firstnumber) ||
				 (end2 >= b->firstnumber + m->invertices)) {
		if (!b->quiet) {
		  ERR_PRINT_STRING("Warning:  Invalid second endpoint of segment %d in %s.",
				 b->firstnumber + i, polyfilename);
		}
	  } else {
		...
		if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
		  if (!b->quiet) {
			ERR_PRINT_STRING("Warning:  Endpoints of segment %d are coincident in %s.",
				   b->firstnumber + i, polyfilename);
		  }
		} else {
  ...
  if (b->convex || !b->poly) {
	...
	if (b->verbose) {
	  ERR_PRINT_STRING("  Enclosing convex hull with segments.");
	}
	markhull(m, b);
  }
}
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Marking concavities (external triangles) for elimination.");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = m->dummytri;
  hulltri.orient = 0;
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Marking neighbors of marked triangles.");
  }
  /* Loop through all the infected triangles, spreading the virus to */
  /*   their neighbors, then to their neighbors' neighbors.          */
  traversalinit(&m->viri);
  virusloop = (triangle **) traverse(&m->viri);
  while (virusloop != (triangle **) nullptr) {
	...
	if (b->verbose > 2) {
	  ...
	  ERR_PRINT_STRING("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)",
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Deleting marked triangles.");
  }
  ...
  while (virusloop != (triangle **) nullptr) {
	...
	for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
	  ...
	  if (testvertex != (vertex) nullptr) {
		...
		if (killorg) {
		  if (b->verbose > 1) {
			ERR_PRINT_STRING("    Deleting vertex (%.12g, %.12g)",
...

{
	...
	for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
	  ...
	  if ((neighbor.tri != m->dummytri) && !infected(neighbor)
		...
		if (b->verbose > 2) {
		  ...
		  ERR_PRINT_STRING("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)",
				 regionorg[0], regionorg[1], regiondest[0], regiondest[1],
				 regionapex[0], regionapex[1]);
		}
		/* Infect the neighbor. */
  ...
  if (b->verbose > 1) {
	ERR_PRINT_STRING("  Unmarking marked triangles.");
  }
  traversalinit(&m->viri);
  virusloop = (triangle **) traverse(&m->viri);
  while (virusloop != (triangle **) nullptr) {
...

{
  ...
  if (!(b->quiet || (b->noholes && b->convex))) {
	ERR_PRINT_STRING("Removing unwanted triangles.");
	if (b->verbose && (holes > 0)) {
	  ERR_PRINT_STRING("  Marking holes for elimination.");
	}
  }
  ...
  if (regions > 0) {
	...
	regiontris = (struct otri *) nullptr;
  }
  ...
  if (regions > 0) {
	if (!b->quiet) {
	  if (b->regionattrib) {
		if (b->vararea) {
		  ERR_PRINT_STRING("Spreading regional attributes and area constraints.");
		} else {
		  ERR_PRINT_STRING("Spreading regional attributes.");
		}
	  } else { 
		ERR_PRINT_STRING("Spreading regional area constraints.");
	  }
	}
...
void precisionerror()
{
  ERR_PRINT_STRING("Try increasing the area criterion and/or reducing the minimum");
  ERR_PRINT_STRING("  allowable angle so that tiny triangles are not created.");
#ifdef SINGLE
  ERR_PRINT_STRING("Alternatively, try recompiling me with double precision");
  ERR_PRINT_STRING("  arithmetic (by removing \"#define SINGLE\" from the");
  ERR_PRINT_STRING("  source file or \"-DSINGLE\" from the makefile).");
#endif /* SINGLE */
}
...

{
  ...
  while ((m->badsubsegs.items > 0) && (m->steinerleft != 0)) {
	...
	while ((encloop != (struct badsubseg *) nullptr) && (m->steinerleft != 0)) {
  ...
  "  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).",
		...
		if (((newvertex[0] == eorg[0]) && (newvertex[1] == eorg[1])) ||
		  ...
		  ERR_PRINT_STRING("Error:  Ran out of precision at (%.12g, %.12g).",
				 newvertex[0], newvertex[1]);
		  ERR_PRINT_STRING("I attempted to split a segment to a smaller size than");
		  ERR_PRINT_STRING("  can be accommodated by the finite precision of");
		  ERR_PRINT_STRING("  floating point arithmetic.");
		  precisionerror();
		  triexit(1);
		}
		...
		if ((success != SUCCESSFULVERTEX) && (success != ENCROACHINGVERTEX)) {
		  ERR_PRINT_STRING("Internal error in splitencsegs():");
		  ERR_PRINT_STRING("  Failure to split a segment.");
		  internalerror();
		}
		if (m->steinerleft > 0) {
...

{
  ...
  if (b->verbose) {
	ERR_PRINT_STRING("  Making a list of bad triangles.");
  }
  traversalinit(&m->triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse(m);
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
	...
	if (b->verbose > 1) {
	  ERR_PRINT_STRING("  Splitting this triangle at its circumcenter:");
	  ERR_PRINT_STRING("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)", borg[0],
	...
	if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
	  ...
	  success = insertvertex(m, b, newvertex, &badotri, (struct osub *) nullptr,
							 1, 1);
	  if (success == SUCCESSFULVERTEX) {
		if (m->steinerleft > 0) {
		  m->steinerleft--;
		...
		undovertex(m, b);
		if (b->verbose > 1) {
		  ERR_PRINT_STRING("  Rejecting (%.12g, %.12g).", newvertex[0], newvertex[1]);
		}
		vertexdealloc(m, newvertex);
	...
	if (errorflag) {
	  if (b->verbose) {
		ERR_PRINT_STRING("  The new vertex is at the circumcenter of triangle");
		ERR_PRINT_STRING("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)",
			   borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
	  }
	  ERR_PRINT_STRING("This probably means that I am trying to refine triangles");
	  ERR_PRINT_STRING("  to a smaller size than can be accommodated by the finite");
	  ERR_PRINT_STRING("  precision of floating point arithmetic.  (You can be");
	  ERR_PRINT_STRING("  sure of this if I fail to terminate.)");
	  precisionerror();
	}
  }
}
...

{
  ...
  if (!b->quiet) {
	ERR_PRINT_STRING("Adding Steiner points to enforce quality.");
  }
  /* Initialize the pool of encroached subsegments. */
  poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK,
		   BADSUBSEGPERBLOCK, 0);
  if (b->verbose) {
	ERR_PRINT_STRING("  Looking for encroached subsegments.");
  }
  /* Test all segments to see if they're encroached. */
  tallyencs(m, b);
  if (b->verbose && (m->badsubsegs.items > 0)) {
	ERR_PRINT_STRING("  Splitting encroached subsegments.");
  }
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
	...
	for (i = 0; i < 4096; ++i ) {
	  m->queuefront[i] = (struct badtriang *) nullptr;
	}
	m->firstnonemptyq = -1;
	...
	m->checkquality = 1;
	if (b->verbose) {
	  ERR_PRINT_STRING("  Splitting bad triangles.");
	}
  ...
  if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
	...
	if (m->badsubsegs.items == 1) {
	  ERR_PRINT_STRING("  one encroached subsegment, and therefore might not be truly"
			 );
	} else {
	  ERR_PRINT_STRING("  %ld encroached subsegments, and therefore might not be truly"
			 , m->badsubsegs.items);
	}
	ERR_PRINT_STRING("  Delaunay.  If the Delaunay property is important to you,");
	ERR_PRINT_STRING("  try increasing the number of Steiner points (controlled by");
	ERR_PRINT_STRING("  the -S switch) slightly and try again.");
  }
}
...

{
  ...
  if (!b->quiet) {
	ERR_PRINT_STRING("Adding vertices for second-order triangles.");
  }
  ...
  m->vertices.deaditemstack = (VOID *) nullptr;
  ...
  while (triangleloop.tri != (triangle *) nullptr) {
	for (triangleloop.orient = 0; triangleloop.orient < 3;
	  ...
	  if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
		...
		if (b->verbose > 1) {
		  ERR_PRINT_STRING("  Creating (%.12g, %.12g).", newvertex[0], newvertex[1]);
		}
...

{
  ...
  do {
	...
	if (result == (char *) nullptr) {
	  ERR_PRINT_STRING("  Error:  Unexpected end of file in %s.", infilename);
	  triexit(1);
	}
...

{
  ...
  if (b->poly) {
	...
	if (!b->quiet) {
	  ERR_PRINT_STRING("Opening %s.", polyfilename);
	}
	*polyfile = fopen(polyfilename, "r");
	if (*polyfile == (FILE *) nullptr) {
	  ERR_PRINT_STRING("  Error:  Cannot access file %s.", polyfilename);
	  triexit(1);
	}
	...
	m->readnodefile = 1;
	infilename = nodefilename;
	*polyfile = (FILE *) nullptr;
  }

  if (m->readnodefile) {
	/* Read the vertices from a .node file. */
	if (!b->quiet) {
	  ERR_PRINT_STRING("Opening %s.", nodefilename);
	}
	infile = fopen(nodefilename, "r");
	if (infile == (FILE *) nullptr) {
	  ERR_PRINT_STRING("  Error:  Cannot access file %s.", nodefilename);
	  triexit(1);
	}
	/* Read number of vertices, number of dimensions, number of vertex */
	/*   attributes, and number of boundary markers.                   */
  ...
  if (m->invertices < 3) {
	ERR_PRINT_STRING("Error:  Input must have at least three input vertices.");
	triexit(1);
  }
  if (m->mesh_dim != 2) {
	ERR_PRINT_STRING("Error:  Triangle only works with two-dimensional meshes.");
	triexit(1);
  }
  if (m->nextras == 0) {
	b->weighted = 0;
  ...
  for (i = 0; i < m->invertices; ++i ) {
	...
	stringptr = findfield(stringptr);
	if (*stringptr == '\0') {
	  ERR_PRINT_STRING("Error:  Vertex %d has no x coordinate.", b->firstnumber + i);
	  triexit(1);
	}
	x = (double) strtod(stringptr, &stringptr);
	stringptr = findfield(stringptr);
	if (*stringptr == '\0') {
	  ERR_PRINT_STRING("Error:  Vertex %d has no y coordinate.", b->firstnumber + i);
	  triexit(1);
	}
  ...
  if (*holes > 0) {
	...
	*hlist = holelist;
	for (i = 0; i < 2 * *holes; i += 2) {
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Hole %d has no x coordinate.",
			   b->firstnumber + (i >> 1));
		triexit(1);
	  } else {
	  ...
	  stringptr = findfield(stringptr);
	  if (*stringptr == '\0') {
		ERR_PRINT_STRING("Error:  Hole %d has no y coordinate.",
			   b->firstnumber + (i >> 1));
		triexit(1);
	  } else {
	...
	if (*regions > 0) {
	  ...
	  *rlist = regionlist;
	  ...
	  for (i = 0; i < *regions; ++i ) {
		...
		stringptr = findfield(stringptr);
		if (*stringptr == '\0') {
		  ERR_PRINT_STRING("Error:  Region %d has no x coordinate.",
				 b->firstnumber + i);
		  triexit(1);
		} else {
		...
		stringptr = findfield(stringptr);
		if (*stringptr == '\0') {
		  ERR_PRINT_STRING("Error:  Region %d has no y coordinate.",
				 b->firstnumber + i);
		  triexit(1);
		} else {
		  regionlist[index++] = (double) strtod(stringptr, &stringptr);
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", nodefilename);
  }
  outfile = fopen(nodefilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", nodefilename);
	triexit(1);
  }
  /* Number of vertices, number of dimensions, number of vertex attributes, */
  /*   and number of boundary markers (zero or one).                        */
  fprintf(outfile, "%ld  %d  %d  %d", outvertices, m->mesh_dim,
  ...
  while (vertexloop != (vertex) nullptr) {
	if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
	  ...
	  if (b->nobound) {
		...
		fprintf(outfile, "    %d", vertexmark(vertexloop));
	  }
#endif /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing triangles.");
  }
  /* Allocate memory for output triangles if necessary. */
  if (*trianglelist == (int *) nullptr) {
	*trianglelist = (int *) trimalloc((int) (m->triangles.items *
											 ((b->order + 1) * (b->order + 2) /
											  2) * sizeof(int)));
  }
  /* Allocate memory for output triangle attributes if necessary. */
  if ((m->eextras > 0) && (*triangleattriblist == (double *) nullptr)) {
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", elefilename);
  }
  outfile = fopen(elefilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", elefilename);
	triexit(1);
  }
  /* Number of triangles, vertices per triangle, attributes per triangle. */
  fprintf(outfile, "%ld  %d  %d", m->triangles.items,
...

{
...
#ifdef TRILIBRARY
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing segments.");
  }
  /* Allocate memory for output segments if necessary. */
  if (*segmentlist == (int *) nullptr) {
	*segmentlist = (int *) trimalloc((int) (m->subsegs.items * 2 *
											sizeof(int)));
  }
  /* Allocate memory for output segment markers if necessary. */
  if (!b->nobound && (*segmentmarkerlist == (int *) nullptr)) {
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", polyfilename);
  }
  outfile = fopen(polyfilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", polyfilename);
	triexit(1);
  }
  /* The zero indicates that the vertices are in a separate .node file. */
  /*   Followed by number of dimensions, number of vertex attributes,   */
  /*   and number of boundary markers (zero or one).                    */
  fprintf(outfile, "%d  %d  %d  %d", 0, m->mesh_dim, m->nextras,
		  1 - b->nobound);
  /* Number of segments, number of boundary markers (zero or one). */
  fprintf(outfile, "%ld  %d", m->subsegs.items, 1 - b->nobound);
#endif /* not TRILIBRARY */
  ...
  while (subsegloop.ss != (subseg *) nullptr) {
	...
	if (b->nobound) {
	  fprintf(outfile, "%4ld    %4d  %4d", subsegnumber,
			  vertexmark(endpoint1), vertexmark(endpoint2));
	} else {
	  fprintf(outfile, "%4ld    %4d  %4d    %4d", subsegnumber,
...

{
...
#ifdef TRILIBRARY
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing edges.");
  }
  ...
  if (!b->nobound && (*edgemarkerlist == (int *) nullptr)) {
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", edgefilename);
  }
  outfile = fopen(edgefilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", edgefilename);
	triexit(1);
  }
  /* Number of edges, number of boundary markers (zero or one). */
  fprintf(outfile, "%ld  %d", m->edges, 1 - b->nobound);
#endif /* not TRILIBRARY */
	...
	for (triangleloop.orient = 0; triangleloop.orient < 3;
	  ...
	  if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
		...
		if (b->nobound) {
		  ...
		  fprintf(outfile, "%4ld   %d  %d", edgenumber,
...
#else /* not TRILIBRARY */
			  fprintf(outfile, "%4ld   %d  %d  %d", edgenumber,
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", vnodefilename);
  }
  outfile = fopen(vnodefilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", vnodefilename);
	triexit(1);
  }
  /* Number of triangles, two dimensions, number of vertex attributes, */
  /*   no markers.                                                     */
  fprintf(outfile, "%ld  %d  %d  %d", m->triangles.items, 2, m->nextras, 0);
#endif /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing Voronoi edges.");
  }
  /* Allocate memory for output Voronoi edges if necessary. */
  if (*vedgelist == (int *) nullptr) {
	*vedgelist = (int *) trimalloc((int) (m->edges * 2 * sizeof(int)));
  }
  *vedgemarkerlist = (int *) nullptr;
  /* Allocate memory for output Voronoi norms if necessary. */
  if (*vnormlist == (double *) nullptr) {
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", vedgefilename);
  }
  outfile = fopen(vedgefilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", vedgefilename);
	triexit(1);
  }
  /* Number of edges, zero boundary markers. */
  fprintf(outfile, "%ld  %d", m->edges, 0);
#endif /* not TRILIBRARY */
	...
	for (triangleloop.orient = 0; triangleloop.orient < 3;
	  ...
	  if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
		...
		if (trisym.tri == m->dummytri) {
		  ...
		  fprintf(outfile, "%4ld   %d  %d   %.17g  %.17g", vedgenumber,
		  ...
		  normlist[coordindex++] = 0.0;
#else /* not TRILIBRARY */
		  fprintf(outfile, "%4ld   %d  %d", vedgenumber, p1, p2);
#endif /* not TRILIBRARY */
		}
		vedgenumber++;
	  }
...

{
...
#ifdef TRILIBRARY
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing neighbors.");
  }
...
#else /* not TRILIBRARY */
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", neighborfilename);
  }
  outfile = fopen(neighborfilename, "w");
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", neighborfilename);
	triexit(1);
  }
  /* Number of triangles, three neighbors per triangle. */
  fprintf(outfile, "%ld  %d", m->triangles.items, 3);
#endif /* not TRILIBRARY */
  ...
  * (int *) (m->dummytri + 6) = -1;
  ...
  while (triangleloop.tri != (triangle *) nullptr) {
	...
	fprintf(outfile, "%4ld    %d  %d  %d", elementnumber,
...

{
  ...
  if (!b->quiet) {
	ERR_PRINT_STRING("Writing %s.", offfilename);
  }
  ...
  if (outfile == (FILE *) nullptr) {
	ERR_PRINT_STRING("  Error:  Cannot create file %s.", offfilename);
	triexit(1);
  }
  /* Number of vertices, triangles, and edges. */
  fprintf(outfile, "OFF\n%ld  %ld  %ld", outvertices, m->triangles.items,
		  m->edges);
  ...
  while (vertexloop != (vertex) nullptr) {
	if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
	  ...
	  fprintf(outfile, " %.17g  %.17g  %.17g", vertexloop[0], vertexloop[1],
			  0.0);
	}
	vertexloop = vertextraverse(m);
  }
  ...
  while (triangleloop.tri != (triangle *) nullptr) {
	...
	fprintf(outfile, " 3   %4d  %4d  %4d", vertexmark(p1) - b->firstnumber,
...

{
  ...
  ERR_PRINT_STRING("Mesh quality statistics:");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; ++i ) {
  ...
  ERR_PRINT_STRING("  Smallest area: %16.5g   |  Largest area: %16.5g",
		 smallestarea, biggestarea);
  ERR_PRINT_STRING("  Shortest edge: %16.5g   |  Longest edge: %16.5g",
		 shortest, longest);
  ERR_PRINT_STRING("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g",
		 minaltitude, worstaspect);

  ERR_PRINT_STRING("  Triangle aspect ratio histogram:");
  ERR_PRINT_STRING("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d",
		 ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
		 aspecttable[8]);
  for (i = 1; i < 7; ++i ) {
	ERR_PRINT_STRING("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d",
		   ratiotable[i - 1], ratiotable[i], aspecttable[i],
		   ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  ERR_PRINT_STRING("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d",
		 ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
		 aspecttable[15]);
  ERR_PRINT_STRING("  (Aspect ratio is longest edge divided by shortest altitude)");

  ERR_PRINT_STRING("  Smallest angle: %15.5g   |  Largest angle: %15.5g",
		 smallestangle, biggestangle);

  ERR_PRINT_STRING("  Angle histogram:");
  for (i = 0; i < 9; ++i ) {
	ERR_PRINT_STRING("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d",
...
{
  ERR_PRINT_STRING("\nStatistics:");
  ERR_PRINT_STRING("  Input vertices: %d", m->invertices);
  if (b->refine) {
	ERR_PRINT_STRING("  Input triangles: %d", m->inelements);
  }
  if (b->poly) {
	ERR_PRINT_STRING("  Input segments: %d", m->insegments);
	if (!b->refine) {
	  ERR_PRINT_STRING("  Input holes: %d", m->holes);
	}
  }

  ERR_PRINT_STRING("\n  Mesh vertices: %ld", m->vertices.items - m->undeads);
  ERR_PRINT_STRING("  Mesh triangles: %ld", m->triangles.items);
  ERR_PRINT_STRING("  Mesh edges: %ld", m->edges);
  ERR_PRINT_STRING("  Mesh exterior boundary edges: %ld", m->hullsize);
  if (b->poly || b->refine) {
	ERR_PRINT_STRING("  Mesh interior boundary edges: %ld",
		   m->subsegs.items - m->hullsize);
	ERR_PRINT_STRING("  Mesh subsegments (constrained edges): %ld",
		   m->subsegs.items);
  }
  ERR_PRINT_STRING("");
  ...
  if (b->verbose) {
	...
	ERR_PRINT_STRING("Memory allocation statistics:");
	ERR_PRINT_STRING("  Maximum number of vertices: %ld", m->vertices.maxitems);
	ERR_PRINT_STRING("  Maximum number of triangles: %ld", m->triangles.maxitems);
	if (m->subsegs.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of subsegments: %ld", m->subsegs.maxitems);
	}
	if (m->viri.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of viri: %ld", m->viri.maxitems);
	}
	if (m->badsubsegs.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of encroached subsegments: %ld",
			 m->badsubsegs.maxitems);
	}
	if (m->badtriangles.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of bad triangles: %ld",
			 m->badtriangles.maxitems);
	}
	if (m->flipstackers.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of stacked triangle flips: %ld",
			 m->flipstackers.maxitems);
	}
	if (m->splaynodes.maxitems > 0) {
	  ERR_PRINT_STRING("  Maximum number of splay tree nodes: %ld",
			 m->splaynodes.maxitems);
	}
	ERR_PRINT_STRING("  Approximate heap memory use (bytes): %ld",
	...
	ERR_PRINT_STRING("Algorithmic statistics:");
	if (!b->weighted) {
	  ERR_PRINT_STRING("  Number of incircle tests: %ld", m->incirclecount);
	} else {
	  ERR_PRINT_STRING("  Number of 3D orientation tests: %ld", m->orient3dcount);
	}
	ERR_PRINT_STRING("  Number of 2D orientation tests: %ld", m->counterclockcount);
	if (m->hyperbolacount > 0) {
	  ERR_PRINT_STRING("  Number of right-of-hyperbola tests: %ld",
			 m->hyperbolacount);
	}
	if (m->circletopcount > 0) {
	  ERR_PRINT_STRING("  Number of circle top computations: %ld",
			 m->circletopcount);
	}
	if (m->circumcentercount > 0) {
	  ERR_PRINT_STRING("  Number of triangle circumcenter computations: %ld",
			 m->circumcentercount);
	}
	ERR_PRINT_STRING("");
  }
...

{
  ...
  if (!b.quiet) {
	gettimeofday(&tv2, &tz);
	if (b.refine) {
	  ERR_PRINT_STRING("Mesh reconstruction");
	} else {
	  ERR_PRINT_STRING("Delaunay");
	}
	ERR_PRINT_STRING(" milliseconds:  %ld", 1000l * (tv2.tv_sec - tv1.tv_sec) +
  ...
  m.infvertex1 = (vertex) nullptr;
  m.infvertex2 = (vertex) nullptr;
  m.infvertex3 = (vertex) nullptr;
  ...
  if (!b.quiet) {
	...
	if (b.usesegments && !b.refine) {
	  ERR_PRINT_STRING("Segment milliseconds:  %ld",
  ...
  if (!b.quiet) {
	gettimeofday(&tv4, &tz);
	if (b.poly && !b.refine) {
	  ERR_PRINT_STRING("Hole milliseconds:  %ld", 1000l * (tv4.tv_sec - tv3.tv_sec) +
...
#ifndef CDT_ONLY
	if (b.quality) {
	  ERR_PRINT_STRING("Quality milliseconds:  %ld",
  ...
  if (vorout != (struct triangulateio *) nullptr) {
  ...
  if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
	if (!b.quiet) {
#ifdef TRILIBRARY
	  ERR_PRINT_STRING("NOT writing vertices.");
#else /* not TRILIBRARY */
	  ERR_PRINT_STRING("NOT writing a .node file.");
#endif /* not TRILIBRARY */
	}
  ...
  if (b.noelewritten) {
	if (!b.quiet) {
#ifdef TRILIBRARY
	  ERR_PRINT_STRING("NOT writing triangles.");
#else /* not TRILIBRARY */
	  ERR_PRINT_STRING("NOT writing an .ele file.");
#endif /* not TRILIBRARY */
	}
  } else {
#ifdef TRILIBRARY
  ...
  if (b.poly || b.convex) {
	...
	if (b.nopolywritten || b.noiterationnum) {
		...
		{
#ifdef TRILIBRARY
			ERR_PRINT_STRING("NOT writing segments.");
#else /* not TRILIBRARY */
			ERR_PRINT_STRING("NOT writing a .poly file.");
#endif /* not TRILIBRARY */
		}
	}
	else
	{
		...
		{
			out->holelist = (double *) nullptr;
			out->regionlist = (double *) nullptr;
		}
#else /* not TRILIBRARY */
	...
	if (!b.quiet)
	{
		...
		ERR_PRINT_STRING("Total running milliseconds:  %ld",
