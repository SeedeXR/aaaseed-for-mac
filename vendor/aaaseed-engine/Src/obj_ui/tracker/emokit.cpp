...
#include <lib_use.h>
AAA_LIB_USE( "hidapi" )
AAA_LIB_USE32( "libmcrypt_a" )


const int32_t SERIAL_CHAR_NB = 64;
const int32_t EMOKIT_KEYSIZE = 16; /* 128 bits == 16 bytes */
...
#define EMOKIT_REPORT_SIZE 9	// is 8 with guiilame headset

//maa eventually to use
/*
enum headset_type { CONSUMER_HEADSET, RESEARCH_HEADSET, SPECIAL_HEADSET };
const unsigned char CONSUMERKEY[EMOKIT_KEYSIZE] =  {0x31,0x00,0x35,0x54,0x38,0x10,0x37,0x42,0x31,0x00,0x35,0x48,0x38,0x00,0x37,0x50};
const unsigned char RESEARCHKEY[EMOKIT_KEYSIZE] =  {0x31,0x00,0x39,0x54,0x38,0x10,0x37,0x42,0x31,0x00,0x39,0x48,0x38,0x00,0x37,0x50};
const unsigned char SPECIALKEY[EMOKIT_KEYSIZE]  = {0x31,0x00,0x35,0x48,0x31,0x00,0x35,0x54,0x38,0x10,0x37,0x42,0x38,0x00,0x37,0x50};
*/
...
typedef unsigned char	emokit_cryto_key[EMOKIT_KEYSIZE];
typedef	wchar_t			emokit_serial[SERIAL_CHAR_NB];		// USB Dongle serial number

struct emokit_device {
	hid_device*						_dev;
	emokit_serial					serial;					// USB Dongle serial number
	int								_is_open;				// Is device currently open
	int								_is_inited;				// Is device current initialized
	MCRYPT							td;						// mcrypt context
	emokit_cryto_key				key;					// crypt key for device
...
#define DO_KEY(ind)	((uint8_t)serial[16-ind]);

EMOKIT_DECLSPEC void emokit_get_crypto_key( emokit_cryto_key& key, emokit_serial& serial, int dev_type )
{
...
{
	emokit_get_crypto_key( s->key , s->serial, dev_type );

	//libmcrypt initialization
	s->td = mcrypt_module_open( MCRYPT_RIJNDAEL_128, nullptr, MCRYPT_ECB, nullptr );
	...
	mcrypt_generic_init( s->td, s->key, EMOKIT_KEYSIZE, nullptr );
	return 0;
}
...
int emokit_open(struct emokit_device* s, int device_vid, int device_pid, unsigned int device_index)
{
	...
	hid_get_serial_number_string(s->_dev, s->serial, SERIAL_CHAR_NB);
	emokit_init_crypto(s, dev_type);
	hid_set_nonblocking( s->_dev, 1);
	return 0;
}
...
EMOKIT_DECLSPEC struct emokit_contact_quality handle_quality( struct emokit_device* s )
{
	...
	{
	case 0:		s->last_quality.F3	=current_contact_quality;	break;
	case 1:		s->last_quality.FC5	=current_contact_quality;	break;
	case 2:		s->last_quality.AF3	=current_contact_quality;	break;
	case 3:		s->last_quality.F7	=current_contact_quality;	break;
	case 4:		s->last_quality.T7	=current_contact_quality;	break;
	case 5:		s->last_quality.P7	=current_contact_quality;	break;
	case 6:		s->last_quality.O1	=current_contact_quality;	break;
	case 7:		s->last_quality.O2	=current_contact_quality;	break;
	case 8:		s->last_quality.P8	=current_contact_quality;	break;
	case 9:		s->last_quality.T8	=current_contact_quality;	break;
	case 10:	s->last_quality.F8	=current_contact_quality;	break;
	case 11:	s->last_quality.AF4	=current_contact_quality;	break;
	case 12:	s->last_quality.FC6	=current_contact_quality;	break;
	case 13:	s->last_quality.F4	=current_contact_quality;	break;
	case 14:	s->last_quality.F8	=current_contact_quality;	break;
	case 15:	s->last_quality.AF4 =current_contact_quality;	break;

	case 64:	s->last_quality.F3	=current_contact_quality;	break;
	case 65:	s->last_quality.FC5	=current_contact_quality;	break;
	case 66:	s->last_quality.AF3	=current_contact_quality;	break;
	case 67:	s->last_quality.F7	=current_contact_quality;	break;
	case 68:	s->last_quality.T7	=current_contact_quality;	break;
	case 69:	s->last_quality.P7	=current_contact_quality;	break;
	case 70:	s->last_quality.O1	=current_contact_quality;	break;
	case 71:	s->last_quality.O2	=current_contact_quality;	break;
	case 72:	s->last_quality.P8	=current_contact_quality;	break;
	case 73:	s->last_quality.T8	=current_contact_quality;	break;
	case 74:	s->last_quality.F8	=current_contact_quality;	break;
	case 75:	s->last_quality.AF4	=current_contact_quality;	break;
	case 76:	s->last_quality.FC6	=current_contact_quality;	break;
	case 77:	s->last_quality.F4	=current_contact_quality;	break;
	case 78:	s->last_quality.F8	=current_contact_quality;	break;
	case 79:	s->last_quality.AF4	=current_contact_quality;	break;
	case 80:	s->last_quality.FC6	=current_contact_quality;	break;				
	default:	break;
	}		
...
{
	struct emokit_frame k;		//todo avoid allocation and copy every time
...
//	printf( "%d\t%d\t%d\t%d\n", (int32_t) k.counter,(int32_t) k.battery, k.gyroX, k.gyroY );
//	printf( "%d\n", (int32_t) s->raw_unenc_frame[29] - 102 );

	k.cq = handle_quality(s);	//todo avoid allocation and copy every time
