...
* 
*   Mâa tried in 2025 April to write to gomzygotit@hotmail.com
*   to get permissions when the code will be on a web site.
*   The email address is not valid anymore, and a search on the web gave no result.
* 
*	HISTORY:
...
//#pragma message( "AAA_VSTOOL is " AAA_STRING(AAA_VSTOOL())  ) 
//#pragma message( "#_MSC_VER is " AAA_STRING(_MSC_VER)  )  
BEGIN_MESSAGE_MAP(CNumericEdit, CEdit)
    ON_MESSAGE(WM_PASTE, OnPaste)
    //ON_EN_CHANGE(IDC_REALIN, &CNumericEdit::PreTranslateMessage)
END_MESSAGE_MAP()
...
CString CNumericEdit::GetClipboardText(void) const
{
    ...
    if(::IsClipboardFormatAvailable(CF_TEXT))
    {
        ...
        {
            HANDLE	hClipBrdData = nullptr;
            if((hClipBrdData = ::GetClipboardData(CF_TEXT)) != nullptr)
            {
...
LRESULT CNumericEdit::OnPaste(WPARAM wParam, LPARAM lParam)
{
    ...
    if(m_chDigitsAfterDecimal)
    {
        ...
        if( (iDecimalPos[0] >= 0 && iDecimalPos[1] >= 0) && 
            (iDecimalPos[0] < int(dwSelStart) || iDecimalPos[0] > int(dwSelEnd)) )
        {
