...
//spread
	float2 force = (float2)(p->pos_b.x, p->pos_b.y-1.)* aaa.p06 * 0.1f;

//repulse
	force += aaa.p04 * mass * 5.f * grad.xy;
//turn
	force += aaa.p05 * mass * 5.f * (float2)(-grad.y,grad.x);
//DAMP
	force += p->vel.xy * aaa.p09;	//	DAMP
	force += (float2)( aaa.p07, aaa.p08 );
	//if( force.y > 0 )
	//	force.y = -force.y;
	p->vel.xy = force;
