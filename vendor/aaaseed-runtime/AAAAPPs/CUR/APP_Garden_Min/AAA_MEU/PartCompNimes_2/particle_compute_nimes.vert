...
#define	nb_u				aaa_vu_int[2]
#define	nb_v				aaa_vu_int[3]

CONST float time			=	aaa_vu_float[0];
CONST float inter_in		=	aaa_vu_float[1];
CONST float inter_range		=	aaa_vu_float[2];
//#define size_gamma			aaa_vu_float[2]
#define ratio_v_factor		aaa_vu_float[2]
#define du					aaa_vu_float[3]
#define dv					aaa_vu_float[4]
...
void main(void)
{
	...
	int iv = gl_InstanceID / nb_u;
	int iu = gl_InstanceID % nb_u;

	float alpha_factor;
#if 1
	vec3 pos_fix = pos_object;
	alpha_factor = smoothstep( 0., .2, 1.-attr.w );
#else
	vec3 pos_fix;
	pos_fix.x = (iu + .5 - nb_u*.5) * du;
	pos_fix.y = (iv + .5 - nb_v*.5) * dv;
	pos_fix.z = 0;

	float ph = time * 3 + attr.y * 10.;
	float inter_part = clamp_01(inter_in + gl_InstanceID*du*dv*.5 + attr.x * inter_range);
	inter_part = pow( inter_part, 2. );
	pos_object.xyz = mix( pos_fix,
						pos_object*.2 + vec3(4*sin(ph)+4,4+attr.z*.0,4*cos(ph)),
						inter_part
						);
...
*/
	out_color = col;
	out_color.a *= gl_Color.a * alpha_factor;
