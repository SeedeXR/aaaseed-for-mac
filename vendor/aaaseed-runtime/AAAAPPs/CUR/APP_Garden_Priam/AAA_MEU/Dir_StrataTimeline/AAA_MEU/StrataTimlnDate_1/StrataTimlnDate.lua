...
function meu:define_ui()
	...
	local textB = param.get_ref( layerB, "bdd" )
	local traB = {}
...
-- -- ---------------SAMPLE CODE SELECTOR PARAMS
-- bu = self:add_selector(	{ix+5,iy+11,	8,sy}, "Font Style")
-- 	bu:set_nb( 10, 1 )
-- 	bu:set_item_text( 1, "vv", "ww", "xx", "yy", "zz", "aa", "bb", "cc", "dd", "ee" )
-- 	bu:set_target_obj_param( layerB, "type" )
...
self:add_slider({ix, iy+3, 4, SY}, "ScaInfoFactor", param.get_ref( transfoB, "scale_factor" ), nil, 1, 0, 8):set_color_back("black") 
...
local function createRectangularHexGrid(numPoints)
	-- 	aaa.print_fn() -- print la fonction et arguments 
    local hexGrid = {}
    local numColumns = math.ceil(math.sqrt(numPoints+numPoints))-1
    local numRows = math.ceil(numPoints / numColumns)-1
    local index = 1
