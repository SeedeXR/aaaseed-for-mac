...
function meu:reset()
	...
	for key, val in PAIRS( self.data ) do
		...
		if nb > 0 then
			for i = 1, nb do
				local pavona = val[ i ]
				pavona.start_anim_t = 0.0
				pavona.animation_name = ""
				pavona.time = 0.0
			end
		end
	end
...
-- PORITE
	...
	local tab =
	{
		TURBIN_FISH =
		{
			{ location = "KL3", position = { x = 14.354, y = -0.01, z = -5.355 }, rotation = { x = .0, y = 65., z = .0 }, scale = 1.0 },
			{ location = "KL4", position = { x = 19.382, y = 0.144, z = -7.721 }, rotation = { x = .0, y = 35., z = .0 }, scale = 1.0 },

			{ location = "KR4", position = { x = 16.039, y = -0.229, z = 5.125 }, rotation = { x = 0.0, y = -20.0, z = 0.0 }, scale = 1.09453 },
			{ location = "KR3", position = { x = 14.113, y = -0.167, z = 5.254 }, rotation = { x = 0, y = 95, z = 0.0 }, scale = 1.0 },

		--	{ fbx_name = "PAVONA_FISH", position = { x = 17.328, y = 0.462, z = -7.7 },
		--	 rotation = { x = .0, y = -145.0, z = .0 }, scale = 1.0 },
		--	{ fbx_name = "PAVONA_FISH", position = { x = 11.268, y = 0.071, z = 5.811 },
...
function meu:define_anim()
	local pavona = {
			GR20 = { comment = "loop defensive",	start_time = 0,		end_time = 323, b_loop = true },
			GR30 = { comment = "sortent",			start_time = 323,	end_time = 445, b_loop = false },
			GR40 = { comment = "peur",				start_time = 445,	end_time = 530, b_loop = false },
			GR50 = { comment = "sortir",			start_time = 530,	end_time = 871, b_loop = false },
		}
	self:update_animation( pavona )
	self.name = "PAVONA_FISH"
	self.anim_cond = pavona
	self.fbx_name = "Fbx_PAVONA_FISH"
end
...
function meu:update_interactivity( obj, location )
	if app.const then
		...
		--if inside( interactivity.x, x_min, x_max ) then
			if interactivity and interactivity.coverage > 0.002 then
	...
	self:draw_layers_begin()
		...
		for key, val in pairs( self.data ) do
			...
			--	local fbx = self:get_meu_by_name_no_error(  self.fbx_name )
				local fbx = self:get_meu_by_name_no_error(  self.name )
				if t and fbx then
					if fbx.b_texs_loaded == false then
						--self:print( "hrtr")
						fbx:draw()
					...
					if false then
						...
						for i = 1, nb do
							...
							--if val[ i ].b_show then
								...
								--if self.__b_do_anim then
									--if i == 1 then
								if app:is_location( obj.location ) or app:is_pc_dev() then
								--	self:print( "obj.location"..obj.location)
									self:update_interactivity( obj, obj.location )
								end
									--elseif i == 2 then
									--	if app:is_location( obj.location ) or app:is_pc_dev() then
									--		self:update_interactivity( obj, "obj.location )
