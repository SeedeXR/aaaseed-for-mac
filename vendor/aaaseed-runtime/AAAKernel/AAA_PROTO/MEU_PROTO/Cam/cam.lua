function meu:define_meu_infos( )
	return { author = "Mâa", date = "2020",
			tags = { "2d", "3D", "Core", "CoreGraphic", "Draw", "Experimental", "VJ" },
			help = "a MEU which concentrate on controlling a camera\n"..
					"set next MEU on camera Curent"
			 }
end
...
function meu:define_cam( cam )
	...
	bu = add( 		0,		0,				"Tra Z",		"tra_offset_z",		-16, 4, false ):set_color_back( "z" )
	iy = iy + SY

	bu = add_size( 	0, 0,		SX/2, SY,	"Tra X",		"tra_offset_x",		-4, 4,	false ):set_color_back( "x" )
		ref.tra_x = param.get_ref( cam, "tra_x" )
	bu = add_size( 	SX/2, 0,	SX/2, SY,	"Tra Y",		"tra_offset_y",		-4, 4,	false ):set_color_back( "y" )
		ref.tra_y = param.get_ref( cam, "tra_y" )
	iy = iy + SY + DY

	bu = add_size( 0,		0,	SX/3,SY,	"Rot X",		"rot_offset_x",		-1, 1,	false ):set_color_back( "x" )
		bu:add_values_def( -.75, -.5, -.25, .25, .5, .75 )
	bu = add_size( SX/3,	0,	SX/3,SY,	"Rot Y",		"rot_offset_y",		-1, 1,	false ):set_color_back( "y" )
		bu:add_values_def( -.75, -.5, -.25, .25, .5, .75 )
	bu = add_size( 2*SX/3,	0,	SX/3,SY,	"Rot Z",		"rot_offset_z", 	-1, 1,	false ):set_color_back( "z" )
		bu:add_values_def( -.75, -.5, -.25, .25, .5, .75 )
	iy = iy + SY + DY
