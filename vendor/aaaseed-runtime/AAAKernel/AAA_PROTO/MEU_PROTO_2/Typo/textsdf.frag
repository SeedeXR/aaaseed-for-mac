...
float	distance_factor		=	aaa_fu_float[0];
...
vec4 get_sdf( vec2 offset )
{
	return texture2D( TEX_SDF, BV.tex_coor[0].xy + offset );
}
...
//   vec2    ts          = textureSize( TEX_SDF, 0 );
	...
	case 1: //Maa
		{
			gcol = get_sdf();
   			float sd = max( min(gcol.r,gcol.g), min( max(gcol.r,gcol.g), gcol.b ) );
			float screen_pix_dist = (sd - 0.5) * distance_factor;	// *  screenPxRange();
   			sd = clamp_01( screen_pix_dist + 0.5 );
			gcol = BV.color;
			//float c = .44;
			//float d = .1;
			gcol.a *= sd;
			//gcol.a *= smoothstep( .44, .54, sd );
		}
		break;
	case 2: //MaaBis
		{
 ...
 //   float opacity = clamp(screenPxDistance + 0.5, 0.0, 1.0);
  //  color = mix(bgColor, fgColor, opacity);
		...
		//gcol.rgb = texture2D( TEX_COLOR, BV.pos_world.xy ).rgb;
			float c = .5;
			float d = .1;
			gcol.a *= smoothstep( c-d, c, sd ) *  smoothstep( c+d, c, sd ) ; 
		}
		break;	
    case 3:
		{
    ...
    case 5:
		...
		break;
	case 6: //Maa text shadow in red: some char have the wrong direction
		{
			gcol = get_sdf();
   			float sd = median( gcol.r, gcol.g, gcol.b );
			gcol = BV.color;
			float c = .44;
			float d = .1;
			sd = smoothstep( c, c+d, sd );
			if( sd < .5  )
			{
				gcol = get_sdf( vec2(-1,1) * .002 );
   				sd = median( gcol.r, gcol.g, gcol.b );
				sd = smoothstep( c, c+d, sd );
				gcol = vec4( 1,0,0, 1);
			}
			gcol.a *= sd;
