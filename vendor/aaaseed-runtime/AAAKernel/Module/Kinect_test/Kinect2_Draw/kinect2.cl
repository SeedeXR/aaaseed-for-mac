...
//get_gradient
	...
	{
		//va = extract_compo( ca, id_bis );
		//vb = extract_compo( cb, id_bis );
		va = extract_lum(ca);
		vb = extract_lum(cb);
	}
...
/*	col_out[id].x = 0;
	...
	col_out[id].w = 1;
	if( id_bis == 0 )
		col_out[id].x = color.x;
	else if( id_bis == 1 )
		col_out[id].y = color.y;
	else
		col_out[id].z = color.z;
*/
	}
...
__kernel void Kinect2_with_img(
							__global Particle*		particles
						,	__global float4*		pos_out
						,	__global float4*		col_out
						,	__global float4*		nor_out
