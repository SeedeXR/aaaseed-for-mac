...
half4 waterfallPS(in sampler2DRect texture, in half2 texcoord)
{
	half4 outColor;
	half2 newTexCoord = texcoord;
	float TimeMod = mod((Time-300)*Speed, 1000);

	if (Time < 300) {
		newTexCoord = texcoord.xy;
	} else {
		TimeMod = max(TimeMod, 0);
		newTexCoord.y = (texsize.y * TimeMod)/700;
//		if (TimeMod > 700) {
//			newTexCoord.y = texsize.y;
//		}
	}

	if (UseYUV) {
		outColor = yuyv_texture_sampler(texture, newTexCoord.xy);
	} else {
		outColor = texture2DRect(texture, newTexCoord.xy);
	}
	return outColor;
}
...
void main()
{
	...
	color.xyz = waterfallPS(tex0, gl_TexCoord[0].xy).xyz;
