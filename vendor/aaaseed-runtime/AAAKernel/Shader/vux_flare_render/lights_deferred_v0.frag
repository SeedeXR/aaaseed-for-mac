...
#	define shadow_color 		aaa_fu_vec4[0]
#endif
...
float normalize_and_compute_falloff( inout vec3 light_direction, in float radius )
{
...
void main()
{
	...
	if( what == 1 )
	{
		out_result = texture( TEX_DIFFUSE_ROUGH, in_tex_uv );
		return;
	}
#endif

	vec2 screenClipPos 	= in_tex_uv * 2.0 - 1.0;
	float depth 		= texture( TEX_DEPTH, in_tex_uv ).r * 2.0 - 1.0;
	...
	if( what != 2 )
	{
		switch( what )
		{
		case 3:		out_result = vec4( vec3(normal_ao.rgb)*.5+.5, 1.0 );	return;
		case 4:		out_result = screen_coord * .5 + .5;					return;
		case 5:		out_result = world_position;							return;
					//vec4( world_position.xyz * .01, 1. );					return;

		case 6:		out_result = vec4( diffuse_rough.rgb, 1.0 );			return;
		case 7: 	//out_result = vec4( abs(normal_ao.rgb), 1f );			return;
					out_result = vec4( normal_ao.rgb, 1.0 );				return;
		case 8:		out_result = vec4( specular_reflection.rgb, 1.0 );		return;
		case 9:		out_result = vec4( emissive.rgb, 1.0 );					return;
		case 10:	out_result = vec4( vec3(depth*.5+.5), 1.0 );			return;

		case 11:	out_result = vec4( diffuse_rough.aaa, 1.0 );			return;
		case 12:	out_result = vec4( normal_ao.aaa, 1.0 );				return;
		case 13:	out_result = vec4( specular_reflection.aaa, 1.0 );		return;
		case 14:	out_result = vec4( emissive.aaa, 1.0 );					return;
		}
	}
#endif
...
#	endif
			...
			//return;
			vec3 shadow_color_contrib = shadow_color.rgb * shadow_contrib;
			shadow_total *= shadow_color_contrib;

			light_contribution *= shadow_color_contrib;
			if( tex_index==0 )
				tex_index = 1;
		//}
		}
...
//POINT
	for( int i = 0; i < u_light_nb.point; ++i )
	{
		...
		float dist_falloff = normalize_and_compute_falloff( light_direction, l.radius );

		light_total += compute_light( surface, light_direction, view_direction, l.color ) * dist_falloff;
	}

//SPOT
	for( int i = 0; i < u_light_nb.spot; ++i )
	{
		st_light_spot l = lights_spot[i];

		vec3 light_direction = l.position - surface.world_position;
		float dist_falloff = normalize_and_compute_falloff( light_direction, l.radius );

		float cone_inner = l.field_of_view;
		float cone_outer = l.field_of_view_decay;

		float cy = cos(cone_outer);
		float cx = 1.0 / ( cos(cone_inner) - cy );

		float cone_falloff = dot( light_direction, -l.direction );
		cone_falloff = saturate( (cone_falloff - cy) * cx );

		light_total += compute_light( surface, light_direction, view_direction, l.color ) * dist_falloff * cone_falloff;
	}

	light_total *= hack_light_factor;

	vec3 ambient_shadow_contrib = mix( vec3(1), shadow_total, hack_shadow_factor );

	if( depth < 1.0 )
		light_total += (surface.ambient_intensity * hack_ambient_offset * ambient_shadow_contrib) * surface.diffuse.rgb;

	out_result = vec4( light_total, 1.0 );
